...
export class LocalFileSystem extends FileSystem {
    ...
    async resolve(path, opts) {
        if (opts?.signal?.aborted)
            throw new FsError('resolve aborted', 'FS_ABORTED');
    ...
    async stat(target, signal) {
        if (signal?.aborted)
            throw new FsError('stat aborted', 'FS_ABORTED');
    ...
    async writeText(target, content, expected, signal) {
        return this.withLock(target.targetKey, async () => {
            ...
            const before = diffable
                ...
                : null;
            await writeFileAtomic(target.targetKey, content, existing?.mode, signal, this.internals, expected?.kind === 'createIfAbsent' ? { displayPath: target.displayPath } : undefined);
