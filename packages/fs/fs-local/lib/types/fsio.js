...
function listingIoError(displayPath, error) {
    ...
    if (isENOENT(error) || isENOTDIR(error))
        return new FsError(`cannot list "${displayPath}": not found`, 'FS_NOT_FOUND', { cause: error });
    /* v8 ignore next -- Windows chmod does not deny directory listing; POSIX covers permission translation. */
    if (isPermissionError(error))
        return new FsError(`cannot list "${displayPath}": permission denied`, 'FS_PERMISSION_DENIED', { cause: error });
...
async function statRegularFile(target, verb, signal) {
    ...
    if (!info.isFile())
        throw new FsError(`cannot ${verb} "${target.displayPath}": not a regular file`, 'FS_NOT_REGULAR_FILE');
    return info;
}
/**
...
export async function readForEdit(absolutePath, displayPath, signal) {
    ...
    if (buffer.includes(0))
        throw new FsError(`cannot edit "${displayPath}": binary file`, 'FS_NOT_TEXT');
