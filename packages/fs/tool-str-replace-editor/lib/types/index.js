...
async function statExisting(ctx, target, command, exec) {
    ...
    if (info.type === 'directory' && command !== 'view') {
        throw new FsError(`The path ${target.displayPath} is a directory and only the \`view\` command can be used on directories`, 'FS_NOT_REGULAR_FILE');
    }
    return info;
}
...
async function listDirectory(ctx, target, maxOutputChars, exec) {
    async function visit(dir, depth) {
        ...
        const rows = [];
        for (const entry of entries.filter(candidate => !candidate.name.startsWith('.')
...
async function replaceInFile(ctx, policy, path, oldStr, newStr, exec) {
    ...
    if (offset === undefined) {
        throw new FsError(`No replacement was performed, old_str \`${oldValue}\` did not appear verbatim in ${target.displayPath}.`, 'FS_EDIT_NOT_FOUND');
    }
    if (offsets.length > 1) {
    ...
    let outcome;
    try {
        outcome = await ctx.fs.writeText(target, before.slice(0, offset) + newValue + before.slice(offset + oldValue.length), intent === undefined
            ? { kind: 'replaceIfVersion', version: info.version }
            : { kind: 'replaceIfVersion', version: intent.version }, exec.signal, sandboxPolicy);
    }
    catch (error) {
        throw policy.mapError(error, sandboxPolicy);
    }
...
async function insertInFile(ctx, policy, path, insertLine, newStr, exec) {
    ...
    const expected = intent === undefined
        ? { kind: 'replaceIfVersion', version: info.version }
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            switch (args.command) {
                ...
                case 'str_replace':
                    return replaceInFile(ctx, policy, args.path, args.old_str ?? undefined, args.new_str, exec);
                case 'insert':
                    return insertInFile(ctx, policy, args.path, args.insert_line ?? undefined, args.new_str ?? undefined, exec);
            }
        },
