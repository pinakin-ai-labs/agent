...
class NotificationSubscriptionImpl {
    ...
    next() {
        ...
        if (this.state.failure !== undefined)
            return Promise.reject(this.state.failure);
...
export class HarnessClient {
    ...
    start() {
        ...
        const maybeSettle = () => {
            if (settled.stderr && settled.exited)
                signalStreamsSettled();
        };
        child.stderr.once('close', () => {
            if (stderrBuffer.length > 0)
                this.appendStderr([stderrBuffer]);
            settled.stderr = true;
            maybeSettle();
        });
    ...
    async request(method, params, timeoutMs) {
        ...
        try {
            ...
            if (error instanceof JsonRpcResponseError || error instanceof RequestTimeoutError)
                throw error;
    ...
    subscribeSessionTree(sessionId) {
        return this.subscribe((notification) => {
            ...
            if (notification.method === 'subagent.started' || notification.method === 'subagent.finished') {
                ...
                if (typeof parentId === 'string' && this.isDescendantOf(parentId, sessionId))
                    return true;
    ...
    closedError(reason) {
        ...
        if (this.exitCode !== undefined)
            parts.push(`exit code: ${String(this.exitCode)}`);
