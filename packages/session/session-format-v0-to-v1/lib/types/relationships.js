...
import { releasedV0Record } from "./validation-helpers.js";
import { RELEASED_V0_EVENT_DISPOSITIONS } from "./dispositions.js";
 ...
 */
export function assertReleasedArtifactRelationships(artifact, extensions = {}) {
    let openTurn = null;
    let openStep = null;
    ...
    const retries = [];
    const retryStarts = new Set();
    const ptcRoots = new Map();
    ...
    for (const event of artifact.events) {
        ...
        if (RELEASED_V0_EVENT_DISPOSITIONS[event.type] === undefined && !extensionStepEvent)
            continue;
        const data = releasedV0Record(event.data, `${event.type} ${event.seq} data`);
        if (SURFACE_TYPES.has(event.type))
            surface = applySurface(surface, event);
        ...
        switch (event.type) {
            ...
            case 'request/header':
                if (openTurn === null)
                    throw new SessionFormatError(`${event.type} is outside an open turn`);
            ...
            case 'tool/code-dispatch': {
                ...
                if (known !== undefined && known !== root)
                    throw new SessionFormatError(`${event.type} changes its rootCallId`);
                ...
                if (event.type === 'tool/code-dispatch-start') {
                    ...
                    if (start === undefined || start.settled)
                        throw new SessionFormatError('tool/code-dispatch has no unique start');
            ...
            case 'llm/retry-started': {
                ...
                if (scheduled === undefined)
                    throw new SessionFormatError('llm/retry-started pairs no prior scheduled attempt');
                ...
                if (retryStarts.has(key))
                    throw new SessionFormatError('llm/retry-started repeats one scheduled attempt');
                retryStarts.add(key);
                break;
            }
            case 'session/title':
            ...
            case 'command/run': {
                ...
                if (commandRuns.has(id))
                    throw new SessionFormatError(`command/run repeats commandId ${id}`);
                commandRuns.add(id);
                break;
            }
            case 'command/done': {
                const id = data['commandId'];
                if (!commandRuns.has(id))
                    throw new SessionFormatError(`command/done ${id} has no prior command/run`);
            ...
            case 'compaction/start':
                if (openCompaction !== undefined)
                    throw new SessionFormatError('compaction/start overlaps an open compaction');
                assertCompactionTurn(data['turn'], openTurn, 'compaction/start');
                openCompaction = {
                    id: data['compactionId'],
                    ...(data['sourceCommandId'] === undefined ? {} : { sourceCommandId: data['sourceCommandId'] }),
                    turn: data['turn'],
                    startSeq: event.seq,
                    summarized: false,
                };
            ...
            case 'compaction/summary':
                ...
                if (openCompaction?.summarized === true)
                    throw new SessionFormatError('compaction/summary repeats');
                assertCurrentSurfaceSpan(surface, data, 'compaction/summary');
                openCompaction = { ...openCompaction, summarized: true };
                break;
            case 'compaction/end':
                assertCompactionOwner(openCompaction, data, 'compaction/end');
                if (data['turn'] !== openCompaction?.turn)
                    throw new SessionFormatError('compaction/end changes its owner turn');
...
function assertRetryChain(retries, data) {
    ...
    if (data['retry'] !== expected)
        throw new SessionFormatError(`llm/retry must use retry ${expected}`);
    if (prior !== undefined
        && prior.data['retryId'] !== data['retryId']) {
        throw new SessionFormatError('llm/retry must preserve retryId across one policy chain');
    }
    if (prior === undefined && retries.some(candidate => candidate.data['retryId'] === data['retryId'])) {
...
function applySurface(surface, event) {
    ...
    if (operation === undefined)
        throw new SessionFormatError(`${event.type} requires a surfaceOp marker`);
    if (operation === 'append')
        return [...surface, event.seq];
    const replace = operation;
    const start = surface.indexOf(replace.start);
    const end = surface.indexOf(replace.end);
    if (start < 0 || end < start)
        throw new SessionFormatError(`${event.type} replacement range is not on the current surface`);
