...
import { assertReleasedArtifactRelationships } from "../relationships.js";
import { assertReleasedArtifactCoordinates, assertReleasedEventPayload, assertReleasedSessionFormatHeader, } from "../validation.js";
const RELEASED_V0_EVENT_TYPE_SET = new Set(Object.keys(RELEASED_V0_EVENT_DISPOSITIONS));
/**
 * Validate one decoded released-v0 source artifact in tests.
 * @param artifact - released-v0 source artifact.
 */
export function assertReleasedV0SourceArtifact(artifact) {
 ...
 */
export function assertNormalizedReleasedV0Artifact(artifact) {
    assertReleasedSessionFormatHeader(artifact.header, 0);
 ...
 */
export function assertReleasedV1MigrationSource(artifact) {
 ...
 */
export function assertReleasedV1Artifact(artifact) {
    assertReleasedSessionFormatHeader(artifact.header, 1);
    assertReleasedArtifactCoordinates(artifact, false, RELEASED_V0_EVENT_TYPE_SET, false, true);
    for (const event of artifact.events) {
        if (RELEASED_V0_EVENT_DISPOSITIONS[event.type] !== undefined)
 ...
 */
export function assertReleasedV1PhysicalArtifact(artifact) {
    assertReleasedSessionFormatHeader(artifact.header, 1);
