...
import { SessionFormatError, SessionFormatUnsupportedMigrationError, sessionFormatCount, sessionFormatSafeInteger, } from '@deepseek-ai/dsh-session-format';
import { isJsonValue } from '@deepseek-ai/dsh-util-values';
import { RELEASED_V0_EVENT_DISPOSITIONS } from "./dispositions.js";
import { assertReleasedPayloadSemantics } from "./payload-validation.js";
import { assertReleasedV0Keys, releasedV0Record } from "./validation-helpers.js";
const HEADER_REQUIRED = ['version', 'id', 'createdAt', 'isSeeded', 'delegationDepth'];
...
const SURFACE_OPTIONAL = ['ignorable', 'sourceEventSeqs', 'surfaceOp'];
 ...
 */
export function assertReleasedSessionFormatHeader(header, version) {
    ...
    if (typeof record['id'] !== 'string')
        throw new SessionFormatError(`format v${version} header id must be a string`);
 ...
 * @param header - detached logical header.
 */
export function assertReleasedV1Header(header) {
 ...
 */
export function restoreReleasedV1Artifact(artifact, knownEventTypes) {
 ...
 */
export function assertReleasedArtifactCoordinates(artifact, allowLegacySteering, knownEventTypes, vocabularyNeutral = false, frozenEnvelope = false) {
    ...
    for (let index = 0; index < artifact.events.length; index += 1) {
        ...
        if (typeof type !== 'string')
            throw new SessionFormatError(`Session event ${index} type must be a string`);
        ...
        if (!currentKnown && !legacy && !ignorableCurrent && !vocabularyNeutral) {
            if (allowLegacySteering) {
                throw new SessionFormatUnsupportedMigrationError(`format v0 contains unknown historical event type ${JSON.stringify(type)} at seq ${index}; migration refuses unknown historical events even when ignorable`);
            }
        ...
        if (frozenEnvelope && surface)
            assertReleasedSurfaceMetadata(record, index, type, 'allow-empty-assistant');
    }
}
/**
 ...
 */
export function assertReleasedSurfaceMetadata(record, seq, type, assistantSources) {
    ...
    if (sources !== undefined) {
        if (!Array.isArray(sources))
            throw new SessionFormatError(`${type} ${seq} sourceEventSeqs must be an array`);
        const seen = new Set();
    ...
    if (replacement['op'] !== 'replace')
        throw new SessionFormatError(`${type} ${seq} surfaceOp must replace`);
    ...
    if (start >= seq || end >= seq)
        throw new SessionFormatError(`${type} ${seq} has an invalid surface replacement`);
}
/**
 ...
 */
export function assertReleasedEventPayload(event, version) {
    ...
    if (event.type === 'subagent/descriptor' && data['version'] !== 3) {
        ...
        if (version === 0) {
            throw new SessionFormatUnsupportedMigrationError(`${event.type} ${event.seq} uses unsupported descriptor version ${descriptorVersion}`);
        }
        return;
    }
