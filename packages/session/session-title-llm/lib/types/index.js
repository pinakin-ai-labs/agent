...
export function resolveSessionTitleLlmConfig(config) {
    ...
    for (const key of Object.keys(value)) {
        if (!CONFIG_KEYS.has(key))
            throw new Error(`session-title-llm: unknown config key "${key}"`);
    }
 ...
 */
export function registerSessionTitleLlmProvider(ctx, config, id, automatic, selectMessages) {
 ...
 */
export async function generateSessionTitleWithLlm(ctx, config, request, selectedMessages, titleProvider) {
