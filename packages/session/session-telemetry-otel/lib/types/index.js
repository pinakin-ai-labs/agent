...
export var SessionTelemetryMode;
...
function isFeedback(session, event) {
    if (event.seq < session.inheritedEventCount)
        return false;
    switch (event.type) {
...
export class OpenTelemetrySessionBackend extends SessionTelemetryBackend {
    ...
    constructor(ctx, config) {
        ...
        ctx.on('feedback/committed', (inspection) => {
            ...
            if (isFeedback(session, committed))
                coordinator.captureSession(session, committed.seq);
        });
    }
    /**
