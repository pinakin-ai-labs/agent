...
import { SessionFormatError, SessionFormatUnsupportedMigrationError, sessionFormatCount, snapshotSessionFormatJson, } from '@deepseek-ai/dsh-session-format';
import { assertReleasedPayloadSemantics, assertReleasedSurfaceMetadata, } from '@deepseek-ai/dsh-session-format-v0-to-v1';
import { RELEASED_V2_EVENT_DISPOSITIONS, RELEASED_V2_EVENT_TYPES } from "../dispositions.js";
import { assertReleasedV2Keys, assertReleasedV2PhysicalArtifact, releasedV2Record, restoreReleasedV2Artifact, } from "../validation.js";
 ...
 */
export function assertReleasedV2Artifact(artifact) {
    ...
    for (const [index, event] of artifact.events.entries()) {
        ...
        if (disposition === undefined) {
            throw new SessionFormatUnsupportedMigrationError(`format v2 contains unknown event type ${JSON.stringify(event.type)} at seq ${index}`);
        }
...
export { assertReleasedV2PhysicalArtifact } from "../validation.js";
function assertPayload(event, disposition) {
    ...
    for (const key of disposition.opaque) {
        if (Object.hasOwn(data, key))
            snapshotSessionFormatJson(data[key], `${event.type} ${event.seq} opaque ${key}`);
    }
