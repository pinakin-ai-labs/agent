 ...
 */
export function normalizedImagePath(root, ref) {
 ...
 */
export async function publishImmutableObject(root, target, data, sha256) {
 ...
 */
export async function publishImmutableObjectStream(root, data, targetFor, signal) {
 ...
 */
export async function publishImmutableAlias(root, source, target, sha256) {
...
async function stageImmutableObject(root, data, signal) {
    ...
    try {
        ...
        if (error instanceof AttachmentError || signal?.aborted === true)
            throw error;
        throw new AttachmentError('Unable to persist attachment.', 'ATTACHMENT_WRITE_FAILED', { cause: error });
    }
}
