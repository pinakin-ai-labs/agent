...
export function orderByModuleGraph(entries) {
    ...
    const visit = (entry) => {
        ...
        for (const name of entry.external ?? []) {
            ...
            if (dependency === entry) {
                throw new Error(`client-modules: "${entry.id}" requests module "${name}" that it answers itself `
...
export class ClientModuleRegistry extends Service {
    ...
    constructor(ctx) {
        ...
        const registerWebCarrier = (webCtx) => {
            webCtx.effect(() => webCtx.webServer.register({ kind: 'prefix', path: '/plugins', handler: this.serveBundle }), 'client-modules: bundle route');
        };
