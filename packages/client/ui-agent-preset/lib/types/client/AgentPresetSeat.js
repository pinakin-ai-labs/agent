...
export function AgentPresetSeat({ load, select, introduced, useAgentPresetSeat, t }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Menu, { open: open, onClose: () => { setOpen(false); }, items: state.options.map((option) => {
                    ...
                    void select(id).then((refusal) => {
                        ...
                        if (refusal === undefined || !pickerVisible.current)
                            return;
                        toastSeq.current += 1;
                        setToast({ seq: toastSeq.current, text: t('switchRefused', { name, reason: refusal }) });
                    });
                }, align: "start", portal: true, className: css.menuAnchor, anchor: (_jsxs("button", { type: "button", className: css.seat, "aria-haspopup": "menu", "aria-expanded": open, title: state.error ?? t('seatHint'), disabled: state.busy, onClick: () => { setOpen(value => !value); }, children: [_jsx(IconAgentPresetOutline16, { className: introducing ? `${css.seatIcon} ${css.introIcon}` : css.seatIcon }), _jsx("span", { className: css.seatLabel, children: shownLabel }), _jsx(IconChevronDownOutline14, { className: css.chevron })] })) }), toast !== null && (_jsx(Toast, { text: toast.text, icon: _jsx(IconWarningOutline16, {}), holdMs: REFUSAL_HOLD_MS, 
