...
import { Button, IconBrowseOutline16, IconCopyOutline16, IconFolderOpenOutline16, IconPlusOutline16, IconTrashOutline16, Modal, Switch, Tag, Tooltip, } from '@deepseek-ai/dsh-client-ui-primitives';
...
export function AgentPresetSection(props) {
    ...
    const creatorButton = props.startCreatorDraft !== undefined && state.rows.some(row => row.id === 'cordis')
        ? (_jsxs("button", { type: "button", className: css.creatorButton, disabled: !state.authorable || !state.showPicker || state.policySaving, title: !state.showPicker
                ? t('enablePickerToCreate')
                : state.authorable ? undefined : t('duplicateUnavailable'), onClick: () => {
                props.startCreatorDraft?.();
                props.close();
            }, children: [_jsx(IconPlusOutline16, { size: 14 }), t('creatorDraft')] }))
        : null;
    return (_jsxs("div", { className: css.section, children: [_jsx("h2", { className: css.title, children: t('nav') }), _jsx("p", { className: css.intro, children: t('sectionIntro') }), _jsxs("div", { className: css.pickerPreference, children: [_jsxs("div", { className: css.pickerPreferenceCopy, children: [_jsxs("span", { className: css.pickerPreferenceTitleRow, children: [_jsx("span", { className: css.pickerPreferenceTitle, children: t('showPicker') }), _jsx(Tag, { children: t('showPickerBeta') })] }), _jsx("p", { className: css.pickerPreferenceDescription, children: t('showPickerDescription') })] }), _jsx(Switch, { checked: state.showPicker, label: t('showPicker'), disabled: state.status !== 'ready' || state.policySaving, onChange: (next) => { void props.setPickerVisible(next); } })] }), state.error === null ? null : _jsx("p", { className: css.error, role: "alert", children: state.error }), [['system', t('builtInGroup')], ['user', t('customGroup')]].map(([trust, heading]) => {
                ...
                if (group.length === 0 && tail === null)
                    return null;
                return (_jsxs("section", { className: css.group, children: [_jsx("h3", { className: css.groupHead, children: heading }), group.length === 0 ? null : (_jsx("ul", { className: css.cards, children: group.map(({ row, text, selectionAction }) => (_jsxs("li", { className: [
                                    css.card,
                                    ...
                                    !state.showPicker && row.broken === undefined && !row.isDefault
                                        ...
                                        : undefined,
                                ].filter(Boolean).join(' '), children: [_jsxs("button", { type: "button", className: css.cardMain, "aria-pressed": row.isDefault, 
                                        ...
                                        disabled: row.isDefault
                                            || (row.broken === undefined && (!state.showPicker || state.policySaving)), "aria-disabled": row.broken !== undefined, "aria-label": `${selectionAction}: ${text.name}`, 
                                        ...
                                        title: selectionAction, onClick: () => {
                                            ...
                                            void props.makeDefault(row.id);
                                                        ...
                                                        : null, _jsx(Tag, { children: row.trust === 'user' ? t('userTrust') : t('builtIn') }), row.isDefault
                                                        ? (_jsx(Tag, { tone: "solid", className: css.inUse, children: state.showPicker ? t('inUse') : t('selectionOffDefault') }))
                                                        : null] }), _jsx(CardDescription, { text: text.description ?? t('noDescription') }), row.broken === undefined
                                                ? null
