 ...
 */
export function canCloseTab(surface, tabId) {
 ...
 */
export function soleDockedTab(state, tabId) {
...
function arriving(state, tabId, toPaneId, otherwise) {
    ...
    if (existing === undefined || existing === tabId)
        return otherwise();
 ...
 */
export function createSidebarRightStore(seed) {
    return defineStore({
        init: () => ({ bySession: {} }),
        actions: {
            ...
            open: (d, sessionId) => { d.bySession = seat(d, sessionId, surface => surface); },
            ...
            openContent: (d, sessionId, intent, settled) => {
                d.bySession = seat(d, sessionId, s => advance(s, (state, mint) => {
                    ...
                    const ops = [...planSetExpanded(state, true)];
                    ...
                    const planned = held !== undefined
                        ? { ops: [{ type: 'focusTab', tabId: held }], tabId: held }
                    ...
                    if (replace !== undefined && replace !== planned.tabId)
            ...
            duplicateTab: (d, sessionId, tabId) => {
                d.bySession = seat(d, sessionId, s => advance(s, (state, mint) => pageKind(state, tabId) !== undefined ? [] : planDuplicateTab(state, mint, tabId).ops, seed));
            },
            ...
            placeTab: (d, sessionId, tabId, toPaneId, index) => {
                d.bySession = seat(d, sessionId, s => advance(s, state => arriving(state, tabId, toPaneId, () => planPlaceTab(state, tabId, toPaneId, index)), seed));
            },
            ...
            dropTab: (d, sessionId, tabId, paneId, zone) => {
                d.bySession = seat(d, sessionId, s => advance(s, (state, mint, makeTab) => {
                    if (zone === 'top' || zone === 'bottom')
                        return [];
                    if (zone !== 'center' && dockPaneIds(state).length >= 2)
                        return [];
                    const plan = () => planDropTab(state, mint, tabId, paneId, zone, makeTab);
            ...
            unfloatPane: (d, sessionId, paneId) => {
                d.bySession = seat(d, sessionId, s => advance(s, (state) => {
                    ...
                    const plan = () => planUnfloatPane(state, paneId);
