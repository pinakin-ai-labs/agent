 ...
 */
export function intentsFor(sessionId, actions, openTab, closeTab) {
    return {
        ...
        floatTab: (tabId, rect) => { actions.floatTab(sessionId, tabId, rect); },
...
function bodiesFor(panel) {
    ...
    return tab => (_jsx(TabSlot, { ...rest, tab: tab, seat: "sidebar.right.pane.tab", fallback: _jsx("p", { className: css.unavailable, "data-sidebar-right-unavailable": true, children: t('tab.unavailable') }) }, tab.id));
}
...
function PanelChrome({ sessionId, fullscreen, autoFullscreen, actions, t }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Tooltip, { label: modeLabel, side: "bottom", delayMs: 500, children: _jsx("button", { type: "button", className: css.iconButton, "aria-label": modeLabel, "data-sidebar-right-mode": next, onClick: () => {
                        if (fullscreen && autoFullscreen)
                            actions.setExpanded(sessionId, false);
                        actions.setMode(sessionId, next);
                    }, children: fullscreen ? _jsx(ExitFullscreenGlyph, {}) : _jsx(FullscreenGlyph, {}) }) }), _jsx(Tooltip, { label: t('chrome.collapse'), side: "bottom", delayMs: 500, children: _jsx("button", { type: "button", className: css.iconButton, "aria-label": t('chrome.collapseAria'), "data-sidebar-right-toggle": true, onClick: () => { actions.toggleExpanded(sessionId); }, children: _jsx(IconPanelLeftOutline16, { className: css.collapseGlyph }) }) })] }));
}
/**
...
function SidebarPanel(panel) {
    ...
    return (_jsx("div", { ref: panelRef, className: css.panel, style: { width: fullscreen ? '100%' : width }, "data-sidebar-right-panel": fullscreen ? 'fullscreen' : 'push', "data-sidebar-right-open": expanded || undefined, "aria-hidden": !expanded || undefined, children: _jsx("div", { className: css.panelBody, children: _jsx(DockSurface, { state: surface.layout, canSplit: canSplit(surface.layout) && dockPaneIds(surface.layout).length < 2, hideSplitWhenBlocked: true, dropZones: "horizontal", minPaneFraction: 0.2, canAddTab: paneId => guideIn(surface.layout, paneId) === undefined, canCloseTab: tabId => canCloseTab(surface, tabId), intents: intentsFor(sessionId, actions, openTab, panel.closeTab), labels: dockLabels(t), renderTab: bodiesFor(panel), renderTabTitle: titlesFor(panel), renderTabMenuItems: (tab, dismiss) => renderSlot('sidebar.right.tab.menu.item', { tab, dismiss }), chrome: _jsx(PanelChrome, { sessionId: sessionId, fullscreen: fullscreen, autoFullscreen: autoFullscreen, actions: actions, t: t }), onRoom: reportRoom }) }) }));
}
...
function Floats(panel) {
    ...
    if (surface.layout.floats.length === 0)
        return null;
    return createPortal(_jsx("div", { className: css.floatHost, "data-sidebar-right-float-host": true, children: _jsx(FloatLayer, { state: surface.layout, canCloseTab: tabId => canCloseTab(surface, tabId), intents: intentsFor(sessionId, actions, openTab, panel.closeTab), labels: dockLabels(t), renderTab: bodiesFor(panel), renderTabTitle: titlesFor(panel) }) }), document.body);
}
/**
 ...
 */
export function RightbarSeat({ sessionId, width, viewportWidth, canShow, useStore, actions, t, renderSlot, syncPresentation, bindService, openTab, closeTab, useTabTypes, useTabNavigation, occurrence, }) {
    ...
    const room = useRef(new Map());
    const reportRoom = useCallback((fits) => { room.current = fits; }, []);
    ...
    useLayoutEffect(() => {
        ...
        const reportWhenCovered = () => {
            ...
            const entering = shown && fullscreen
                ? panelRef.current.getAnimations().filter(animation => 'transitionProperty' in animation && animation.transitionProperty === 'transform'
    ...
    useEffect(() => bindService({ sessionId, actions, surfaces, canSplitPane: paneId => room.current.get(paneId)?.row !== false }), [bindService, sessionId, actions, surfaces]);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(SidebarPanel, { ...panel, width: width, panelRef: panelRef }), _jsx(Floats, { ...panel })] }));
}
