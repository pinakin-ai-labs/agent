...
function AppIcon({ id, url, size }) {
    ...
    if (failed) {
        return (_jsx("svg", { width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 1.8, className: css.icon, "aria-hidden": true, children: _jsx("rect", { x: 3, y: 3, width: 18, height: 18, rx: 5 }) }));
    }
    return (_jsx("img", { src: url, width: size, height: size, className: css.icon, alt: "", "aria-hidden": true, draggable: false, onError: () => {
            failedIcons.add(id);
            setFailed(true);
        } }));
}
 ...
 */
export function OpenInAppAction(props) {
    ...
    const busyTimer = useRef(undefined);
    ...
    if (currentEntry === undefined || cwd === undefined || cwd === '')
        return null;
    ...
    const items = apps.map(entry => ({
        id: entry.id,
        label: t(entry.labelKey),
        icon: _jsx(AppIcon, { id: entry.id, url: props.iconUrl(entry.id), size: 18 }),
    }));
    return (_jsx(Menu, { open: open, align: "end", dense: true, selection: "fill", onClose: () => { setOpen(false); }, items: items, selectedId: current, onSelect: (id) => {
            setOpen(false);
            ...
            props.choose(id);
            launch(id);
        }, anchor: (_jsxs("div", { className: css.split, children: [_jsx(Tooltip, { label: phase === 'error' ? t('open.error') : t('open.tooltip'), side: "bottom", children: _jsx("button", { type: "button", className: css.main, "data-state": phase, disabled: phase === 'busy', "aria-label": title, onClick: () => { launch(current); }, children: _jsx(AppIcon, { id: current, url: props.iconUrl(current), size: 15 }) }) }), _jsx("button", { type: "button", className: css.chevron, "aria-expanded": open, "aria-haspopup": "menu", title: t('menu.toggle'), "aria-label": t('menu.toggle'), onClick: () => { setOpen(value => !value); }, children: _jsx(IconChevronDownOutline14, { size: 11 }) })] })) }));
}
//# sourceMappingURL=OpenInAppAction.js.map