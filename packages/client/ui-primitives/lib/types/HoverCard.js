 ...
 */
export function HoverCard({ anchor, content, openDelayMs = 500, disabled = false, copyText, copyLabel, copiedLabel, }) {
    ...
    return (_jsxs("span", { ref: rootRef, className: css.root, onPointerEnter: () => {
            if (disabled)
                return;
        ...
        onPointerDownCapture: (e) => {
            ...
            cancelClose();
            close();
        }, children: [anchor, open && copyable && _jsx("span", { className: css.status, role: "status", children: copied ? copiedLabel : '' }), card !== false && createPortal(card, document.body)] }));
}
