 ...
 */
export function Tooltip({ label, side = 'right', delayMs = 0, disabled = false, maxWidth, children }) {
    ...
    useLayoutEffect(() => {
        ...
        const fit = () => {
            ...
            if (r.left + dx < EDGE_MARGIN)
                dx = EDGE_MARGIN - r.left;
    ...
    const hide = () => {
        cancelShow();
        if (!triggers.current.hover && !triggers.current.focus)
            setPos(null);
    };
    return (_jsxs(_Fragment, { children: [cloneElement(children, {
            ...
            }), pos !== null && (_jsx("span", { ref: bubble, className: css.bubble, "data-side": placement, style: { left: pos.x, top: y, ...maxWidth === undefined ? {} : { maxWidth } }, role: "tooltip", children: resolvedLabel }))] }));
}
