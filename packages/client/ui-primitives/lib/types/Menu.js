...
export function Menu({ open, anchor, items, selectedId, selectedIds, onSelect, onClose, align = 'start', side = 'bottom', portal = false, closeOnPointerLeave = false, dense = false, compact = false, autoFocus = false, selection = 'check', getAnchorRect, footer, className }) {
    ...
    useEffect(() => {
        ...
        const onKeyDown = (e) => {
            ...
            if (!autoFocus || !['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(e.key))
                return;
    ...
    const renderEntry = (entry) => {
        ...
        return (_jsxs("div", { className: css.itemWrap, onMouseEnter: () => { setOpenSubmenuId(hasSub ? entry.id : null); }, onMouseLeave: () => { setOpenSubmenuId(null); }, children: [_jsxs("button", { type: "button", role: "menuitem", className: clsx(css.item, selected && (selection === 'fill' ? css.selectedFill : css.selected), entry.danger === true && css.danger), disabled: entry.disabled, "aria-haspopup": hasSub ? 'menu' : undefined, "aria-expanded": hasSub ? subOpen : undefined, onFocus: () => { setOpenSubmenuId(hasSub ? entry.id : null); }, onClick: () => {
                        if (hasSub) {
                            setOpenSubmenuId(entry.id);
                            return;
                        }
                        onSelect(entry.id);
                    }, children: [entry.icon !== undefined && _jsx("span", { className: css.itemIcon, children: entry.icon }), _jsx("span", { className: css.itemLabel, children: entry.label }), selected && selection === 'check' && _jsx(IconCheckOutline16, { className: css.check })] }), subOpen && entry.submenu !== undefined && (_jsx("div", { className: clsx(css.submenu, compact && css.compactList), role: "menu", children: entry.submenu.map(sub => (_jsxs("button", { type: "button", role: "menuitem", className: css.item, disabled: sub.disabled, onClick: () => { onSelect(sub.id); }, children: [sub.icon !== undefined && _jsx("span", { className: css.itemIcon, children: sub.icon }), _jsx("span", { className: css.itemLabel, children: sub.label })] }, sub.id))) }))] }, entry.id));
    };
    ...
    const list = open && (_jsxs("div", { ref: listRef, className: clsx(css.list, dense && css.denseList, compact && css.compactList, scrollable && css.scrollable, portal && css.portal, side === 'top' && !portal && css.sideTop, align === 'end' && !portal && css.alignEnd), style: portal ? fixedPos ?? MEASURE_STYLE : undefined, role: "menu", 
        // React portals bubble synthetic events through the REACT tree: without
        // this stop, an item click re-fires the anchor row's own onClick
        // (open/toggle) after onSelect.
        onClick: (e) => { e.stopPropagation(); }, children: [_jsx("div", { className: css.viewport, role: "presentation", children: items.map(renderEntry) }), footer !== undefined && footer.length > 0 && (_jsx("div", { className: css.footer, role: "presentation", children: footer.map(renderEntry) }))] }));
