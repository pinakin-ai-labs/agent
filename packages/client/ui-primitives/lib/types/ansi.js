 ...
 */
const WIDE_CHAR = new RegExp('\\p{Script=Han}|\\p{Script=Hiragana}|\\p{Script=Katakana}|\\p{Script=Hangul}'
...
function foldSgr(state, params) {
    ...
    for (let index = 0; index < codes.length; index++) {
        ...
        if (!next.attrs.includes(code))
            next = { ...next, attrs: [...next.attrs, code] };
    }
    return next;
}
/**
    ...
    const consume = (chunk) => {
        for (const char of chunk) {
            ...
            if (ZERO_WIDTH.test(char)) {
                ...
                if (base !== undefined)
                    columns[cursor - 1] = { sgr: base.sgr, char: base.char + char };
                continue;
            }
    ...
    for (const match of line.matchAll(csi)) {
        ...
        if (final === 'K') {
            ...
            if (mode === '1')
                ...
                columns.length = mode === '2' ? 0 : cursor;
            continue;
        }
...
function applyCursorMovements(text) {
    ...
    for (const raw of text.split('\n')) {
        ...
        for (const match of line.matchAll(SGR_SEQUENCE))
            sgr = foldSgr(sgr, String(match[1]));
    }
