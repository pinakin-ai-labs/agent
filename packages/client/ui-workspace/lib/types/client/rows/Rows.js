...
import { HoverCard, IconAlarmClockOutline16, IconArchiveOutline20, IconBranchOutline16, IconEditOutline16, IconEllipsisOutline16, IconFolderClose16, IconFolderOpen16, IconPlusOutline16, IconTrashOutline16, IconTriangleRightFill14, Menu, relativeTime, StateDot, } from '@deepseek-ai/dsh-client-ui-primitives';
...
function ActiveScheduleIndicator({ t, search = false }) {
    ...
    return (_jsx("span", { className: clsx(css.scheduleIndicator, search && css.searchScheduleIndicator), role: "img", "aria-label": label, title: label, children: _jsx(IconAlarmClockOutline16, {}) }));
}
...
export function SearchResultItem({ result, currentId, onOpen, t }) {
    ...
    return (_jsxs("button", { type: "button", className: clsx(css.searchResultRow, selected && css.selected), role: "treeitem", "aria-selected": selected, onClick: () => { onOpen(result.id); }, children: [_jsxs("span", { className: css.searchResultHeading, children: [_jsx("span", { className: css.slot, children: (primaryStatus.state !== 'done' || result.completed) && (_jsx(SessionStatusDots, { statuses: statuses })) }), _jsx("span", { className: css.searchResultTitle, children: result.title }), result.hasActiveSchedule && _jsx(ActiveScheduleIndicator, { t: t, search: true })] }), _jsxs("span", { className: css.searchResultMeta, children: [_jsx("span", { className: css.searchResultWorkspace, children: result.workspace || t('group.ungrouped') }), result.snippet !== undefined && (_jsx("span", { className: css.searchResultSnippet, children: result.snippet }))] })] }));
}
/**
 ...
 */
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onReveal, drag, flat = false, t, }) {
    const row = node;
    ...
    const ownRow = (_jsxs("div", { ref: rowRef, className: clsx(css.sessionRow, selected && css.selected, menuOpen && css.menuOpen, flat && !showStatus && css.flatSessionRowWithoutStatus, drag?.marker === 'before' && css.dropBefore, drag?.marker === 'after' && css.dropAfter), role: "treeitem", "aria-selected": selected, onClick: () => { onOpen(node.id); }, draggable: draggable, onDragStart: drag === undefined || row.blank
            ? undefined
            : (e) => {
                e.dataTransfer.effectAllowed = 'move';
                e.dataTransfer.setData('text/plain', node.id);
                drag.start();
            }, onDragEnd: drag === undefined || row.blank ? undefined : drag.end, onDragOver: drag === undefined
            ? undefined
            : (e) => {
                if (!drag.active)
                    return;
            ...
            : (e) => {
                ...
                e.preventDefault();
                drag.drop(rowHalf(e));
            }, children: [(!flat || showStatus) && (_jsx("span", { className: css.slot, children: showStatus && _jsx(SessionStatusDots, { statuses: statuses }) })), _jsx("span", { className: css.title, children: title }), row.hasActiveSchedule && _jsx(ActiveScheduleIndicator, { t: t }), !row.blank && _jsx("span", { className: css.time, children: timeLabel(row.updatedAt, now, t) }), !row.blank && (_jsx("span", { className: css.rowActions, children: _jsx(Menu, { open: menuOpen, onClose: () => { setMenuOpen(false); }, items: sessionMenuItems, onSelect: (id) => {
                        setMenuOpen(false);
                        if (id === 'rename')
