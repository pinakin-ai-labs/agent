...
import { deriveTrajectoryTimeline, formatTimelineOffset, } from "./timeline.js";
...
export const TrajectoryTimeline = memo(function TrajectoryTimeline({ t, turns, mode, range, hasEarlierRecords = false, onLoadEarlier, selectedIndex = null, searchMatchIndexes = null, onRangeChange, onRecordSelect, onRecordFocus, }) {
    ...
    useEffect(() => {
        ...
        setViewport(current => current !== null && (current.end < model.start || current.start > model.end)
            ? null
            : current);
    }, [model]);
    useEffect(() => {
        if (model === null || selectedIndex === null)
            return;
        ...
        setViewport((current) => {
            if (current === null)
                return current;
            if (selectedSpan.end > current.start
    ...
    useEffect(() => {
        ...
        const onWheel = (event) => {
            ...
            const nextDuration = Math.min(fullDuration, Math.max(Math.min(mode === 'sequence' ? MINIMUM_ZOOM_OPERATIONS : 20, fullDuration), domainDuration * Math.exp(event.deltaY * 0.0015)));
    ...
    return (_jsx("section", { ref: rootRef, className: css.root, "aria-label": t('timeline.aria'), children: _jsxs("div", { className: css.plot, children: [_jsx(LaneLabels, { t: t }), _jsxs("div", { ref: trackRef, className: css.track, "data-panning": panning || undefined, "aria-label": t('timeline.overviewAria'), tabIndex: 0, onKeyDown: onKeyDown, onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: onPointerEnd, onPointerCancel: onPointerCancel, onPointerLeave: () => {
                        ...
                        event.preventDefault();
                                ...
                                '--trajectory-hover-left': `${hover.fraction * 100}%`,
                                    ...
                                    } }), _jsx("div", { className: css.selectionEdges, "data-dragging": draft === null ? undefined : 'true', "aria-hidden": "true", style: {
                                        '--trajectory-selection-left': `${visibleRange.start * 100}%`,
                                        '--trajectory-selection-width': `${(visibleRange.end - visibleRange.start) * 100}%`,
                                    } })] })), _jsx("div", { className: css.turnBoundaries, "data-animate-viewport": animateViewport || undefined, "aria-hidden": "true", style: projectedDomainStyle, children: model.turnBoundaries
                                .filter(boundary => boundary.time > model.start
                                && boundary.time >= domainStart
                                && boundary.time <= domainStart + domainDuration)
                                .map(boundary => (_jsx("span", { className: css.turnBoundary, "data-turn": boundary.turn, style: {
                                    '--trajectory-turn-left': `${(boundary.time - model.start) / fullDuration * 100}%`,
                                } }, boundary.turn))) }), _jsx("div", { className: css.lanes, "data-animate-viewport": animateViewport || undefined, "data-timeline-domain": true, style: projectedDomainStyle, children: model.spans
                                .filter(span => span.index === selectedIndex
...
//# sourceMappingURL=TrajectoryTimeline.js.map