...
import { TrajectoryTable, } from "./TrajectoryTable.js";
import { TrajectoryToolbar } from "./TrajectoryToolbar.js";
import { TrajectoryTimeline } from "./TrajectoryTimeline.js";
import { appendTrajectoryPartialLayout, deriveTrajectoryLayout, } from "./layout.js";
...
export function TrajectoryView({ useSession, useTrajectory, useDuration, loadOlder, loadImage, setActualDuration, viewRequest, completeViewRequest, renderSlot, t, jsonStringWrapping, }) {
    ...
    useEffect(() => {
        ...
        if (searchIndexTimer.current !== null)
            return;
    ...
    useEffect(() => () => {
        if (searchIndexTimer.current !== null)
            clearTimeout(searchIndexTimer.current);
    }, []);
    const streamingCells = useMemo(() => partialSearchTurns.flatMap(turn => turn.groups.flatMap(group => group.cells)), [partialSearchTurns]);
    const searchMatchRecordIds = useMemo(() => searchIndex.search(searchQuery), [searchIndex, searchIndexRevision, searchQuery]);
    const searchMatchIndexes = useMemo(() => {
        if (searchMatchRecordIds === null)
            return null;
        ...
        for (const turns of searchLayouts) {
            for (const turn of turns) {
                for (const group of turn.groups) {
                    for (const cell of group.cells) {
                        if (searchMatchRecordIds.has(trajectoryRecordId(cell)))
    ...
    const timelineFocusIndexes = useMemo(() => timelineRange === null
        ? null
        : trajectoryTimelineFocusIndexes(timelineTurns, timelineRange, timelineMode), [timelineMode, timelineRange, timelineTurns]);
    const handleRecordSelect = useCallback((index) => {
        if (timelineFocusIndexes !== null
            && !timelineFocusIndexes.has(index)) {
            setTimelineSelection(null);
        }
    }, [timelineFocusIndexes]);
    const handleTimelineRangeChange = useCallback((range) => {
        setTimelineSelection(range);
    }, []);
    const handleTimelineRecordSelect = useCallback((index) => {
        setTimelineSelection(null);
        setTimelineRecordSelection({ index });
        setSelectedTimelineIndex(index);
    }, []);
    const handleTimelineRecordFocus = useCallback((index) => {
        setTimelineRecordFocus({ index });
    }, []);
    const collapsibleTurnIds = useMemo(() => timelineTurns
        .filter(turn => turn.turn !== null
        &&
            turn.groups.reduce((count, group) => count + group.cells.filter(cell => cell.requestOnly !== true && cell.kind !== 'system').length, 0) > 1)
        .flatMap(turn => turn.turn === null ? [] : [turn.turn]), [timelineTurns]);
    ...
    const toggleTurn = (turn) => {
        setCollapsedTurns((current) => {
            ...
            if (collapsed.has(turn))
                collapsed.delete(turn);
            else
                collapsed.add(turn);
            return collapsed;
        });
    ...
    const toggleAllAssistants = () => {
        setCollapsedAssistants((current) => {
            ...
            if (allAssistantsCollapsed) {
                for (const index of collapsibleAssistantIds)
