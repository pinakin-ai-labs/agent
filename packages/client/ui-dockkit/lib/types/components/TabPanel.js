...
import { getTab } from "../engine/tree.js";
...
function SplitGlyph() {
    return (_jsx("svg", { width: "16", height: "16", viewBox: "0 0 16 16", fill: "none", "aria-hidden": "true", children: _jsx("path", { fillRule: "evenodd", clipRule: "evenodd", d: `${PANEL_FRAME}M7.31989 1.88307H8.68012V14.1169H7.31989V1.88307Z`, fill: "currentColor" }) }));
}
/**
...
function ZoneGlyph({ zone }) {
    return (_jsxs("svg", { width: "16", height: "16", viewBox: "0 0 16 16", fill: "none", "aria-hidden": "true", children: [_jsx("path", { fillRule: "evenodd", clipRule: "evenodd", d: PANEL_FRAME, fill: "currentColor" }), _jsx("path", { d: ZONE_FILL[zone], fill: "currentColor" })] }));
}
/**
...
function DockHint({ zone, active, labels }) {
    return (_jsx("div", { className: css.dockHint, "data-dockkit-dock-zone": zone, "data-dockkit-drop-active": active || undefined, children: _jsxs("div", { className: css.dockHintCard, children: [_jsx(ZoneGlyph, { zone: zone }), _jsx("span", { className: css.dockHintLabel, children: labels.dropZone[zone] })] }) }));
}
/**
...
function useStripScrollFades(box, tabs) {
    useLayoutEffect(() => {
        ...
        const apply = () => {
            ...
            if (sides === undefined)
                delete element.dataset.dockkitStripScroll;
            else
                element.dataset.dockkitStripScroll = sides;
        };
        apply();
...
function useActiveChipInView(box, chips, tabs, activeTabId) {
    useLayoutEffect(() => {
        ...
        if (element === null || chip === undefined)
            return;
        const bounds = element.getBoundingClientRect();
        const rect = chip.getBoundingClientRect();
        if (rect.left < bounds.left)
            element.scrollLeft += rect.left - bounds.left - STRIP_FADE;
        else if (rect.right > bounds.right)
            element.scrollLeft += rect.right - bounds.right + STRIP_FADE;
...
export function TabPanel({ state, pane, callbacks }) {
    ...
    const activate = (tabId) => {
        if (state.activePaneId === pane.id && pane.activeTabId === tabId)
            return;
    ...
    return (_jsxs("section", { className: css.pane, "data-dockkit-pane": pane.id, "data-dockkit-pane-active": state.activePaneId === pane.id || undefined, 
        ...
        onClick: () => {
            if (state.activePaneId === pane.id)
                return;
            callbacks.onFocusPane(pane.id);
        }, children: [_jsxs("div", { className: css.tabStrip, role: "tablist", "data-dockkit-strip": pane.id, children: [_jsxs("div", { ref: stripTabs, className: css.stripTabs, role: "presentation", "data-dockkit-strip-tabs": pane.id, children: [pane.tabs.map((tabId, index) => {
                                ...
                                return (_jsxs(Fragment, { children: [(index > 0 || stripIndex === index) && (_jsx("div", { className: clsx(css.slot, stripIndex === index && css.slotCaret), "data-dockkit-caret": stripIndex === index ? index : undefined })), _jsxs("div", { role: "tab", "aria-selected": selected, tabIndex: selected ? 0 : -1, className: clsx(css.tab, selected && css.tabActive, quiet && css.tabQuiet, callbacks.draggingTabId === tabId && css.tabDragging), "data-dockkit-tab": tabId, "data-dockkit-tab-quiet": quiet || undefined, ref: (element) => {
                                                if (element === null)
                                                    chips.delete(tabId);
                                                else
                                                    chips.set(tabId, element);
                                            }, 
                                            ...
                                            onPointerDown: (event) => {
                                                ...
                                                setMenu(current => current?.tabId === tabId ? undefined : { tabId, anchor });
                                            }, children: [_jsx(TabTitle, { children: callbacks.renderTabTitle?.(tab) ?? tab.title }), closable && (_jsx("button", { type: "button", className: css.tabClose, "aria-label": callbacks.labels.closeTab, "data-dockkit-tab-close": tabId, 
                                                    ...
                                                    }, children: _jsx(IconCloseFill14, { size: 14 }) })), menu?.tabId === tabId && (_jsx(TabMenu, { labels: callbacks.labels, anchor: menu.anchor, onClose: closable ? () => { setMenu(undefined); callbacks.onCloseTab(tabId); } : undefined, onDismiss: () => { setMenu(undefined); }, extras: callbacks.renderTabMenuItems?.(tab, () => { setMenu(undefined); }) }))] })] }, tabId));
                            }), stripIndex === pane.tabs.length && _jsx("div", { className: clsx(css.slot, css.slotCaret), "data-dockkit-caret": stripIndex })] }), callbacks.canAddTab(pane.id) && (_jsx(Tooltip, { label: callbacks.labels.addTab, side: "bottom", delayMs: 500, children: _jsx("button", { type: "button", className: css.addTab, "aria-label": callbacks.labels.addTab, "data-dockkit-add-tab": pane.id, onClick: (event) => {
                                event.stopPropagation();
                                callbacks.onAddTab(pane.id);
                            }, children: _jsx(IconPlusOutline16, { size: 14 }) }) })), _jsx("div", { className: css.stripFill, "data-dockkit-strip-fill": true }), !(callbacks.hideSplitWhenBlocked && block !== undefined) && (_jsx(Tooltip, { label: callbacks.labels.splitPane, side: "bottom", delayMs: 500, disabled: block !== undefined, children: _jsx("button", { type: "button", className: css.iconButton, "aria-label": callbacks.labels.splitPane, 
                            // Disabled buttons fire no hover events, so the blocked reason
                            // stays a native title.
                            title: block === undefined ? undefined : splitBlockedTitle(callbacks.labels, block), disabled: block !== undefined, "data-dockkit-split-button": pane.id, "data-dockkit-split-blocked": block, onClick: (event) => {
                                event.stopPropagation();
                                callbacks.onSplitPane(pane.id);
                            }, children: _jsx(SplitGlyph, {}) }) })), pane.id === callbacks.chromePaneId && callbacks.chrome !== undefined && (_jsx("div", { className: css.stripChrome, "data-dockkit-strip-chrome": true, onClick: (event) => { event.stopPropagation(); }, children: callbacks.chrome }))] }), _jsxs("div", { className: css.paneBody, children: [active === undefined
                        ? _jsx("p", { className: css.empty, children: callbacks.labels.emptyPane })
                        : callbacks.renderTab(active), zone !== undefined && (_jsxs(_Fragment, { children: [_jsx("div", { className: css.dockScrim, "data-dockkit-dock-scrim": true }), callbacks.horizontalDrops && zone !== 'center'
                                ? _jsxs(_Fragment, { children: [_jsx(DockHint, { zone: "left", active: zone === 'left', labels: callbacks.labels }), _jsx(DockHint, { zone: "right", active: zone === 'right', labels: callbacks.labels })] })
                                : _jsx(DockHint, { zone: zone, active: true, labels: callbacks.labels })] }))] })] }));
}
