import { assertNever, dockPaneIds } from "./tree.js";
...
export const FLOAT_DEFAULT_SIZE = { width: 380, height: 300 };
/** Smallest size a floating panel may be resized to, in CSS pixels. */
export const FLOAT_MIN_SIZE = { width: 220, height: 140 };
 ...
 */
export function dockPaneCount(state) {
...
export const DOCK_ZONES = ['center', 'top', 'right', 'bottom', 'left'];
/**
 ...
 */
export function zoneAt(x, y, edge = DOCK_EDGE_FRACTION) {
    let zone = 'left';
    let distance = x;
    if (1 - x < distance) {
        zone = 'right';
 ...
 */
export function zoneSplit(zone) {
    switch (zone) {
 ...
 */
export function clampSizes(sizes, minimum = MIN_PANE_FRACTION) {
    if (sizes.length === 0)
        return [];
