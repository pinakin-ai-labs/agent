import { assertNever, entriesOf, findParent, findTabPane, firstDockPaneId, floatIndex, floatRect, getPane, getSplit, getTab, insertAt, keysOf, neighbourTabId, normalizeSizes, onlyTabId, paneWithTabs, removeAt, replaceInParent, withNodes, withTabs, } from "./tree.js";
...
function focusSnapshot(state, paneIds) {
    ...
    for (const id of paneIds)
        paneActiveTabs[id] = getPane(state, id).activeTabId;
    return {
        type: 'restoreFocus',
...
function reseatFocus(state, removedPaneId) {
    if (state.activePaneId !== removedPaneId)
        return state;
...
function applyMerge(state, op) {
    ...
    if (pane.tabs.length > 0)
        throw new Error('layout: merge requires an empty pane');
    ...
    const parent = findParent(state, op.paneId);
    if (parent === undefined)
        throw new Error('layout: the docked root pane cannot be merged');
    const index = parent.children.indexOf(op.paneId);
    ...
    if (siblingId === undefined)
        throw new Error('layout: merge found a split without a sibling');
...
function applyOpenTab(state, op) {
    const pane = getPane(state, op.paneId);
    if (pane.host !== 'dock')
        throw new Error('layout: openTab requires a docked pane');
...
function applyInsertTab(state, op) {
    const pane = getPane(state, op.paneId);
    if (pane.host !== 'dock')
        throw new Error('layout: insertTab requires a docked pane');
    assertFreeTab(state, op.tab.id);
    const focus = focusSnapshot(state, [pane.id]);
...
function applyCloseTab(state, op) {
    ...
    if (pane.host === 'float') {
        ...
        const dropped = withTabs(withNodes({ ...state, floats: removeAt(state.floats, index) }, { [pane.id]: null }), { [op.tabId]: null });
        return {
...
function applyInsertPane(state, op) {
    ...
    if (op.pane.tabs.length !== op.tabs.length)
        throw new Error('layout: insertPane tab records do not match the pane');
    if (op.pane.host === 'dock' && op.tabs.length > 0)
        throw new Error('layout: insertPane returns a docked pane empty');
    const tabUpdates = {};
    ...
    switch (attach.mode) {
        case 'child': {
            ...
            if (attach.sizes.length !== children.length)
                throw new Error('layout: insertPane sizes do not match the split');
        ...
        case 'float': {
            if (op.pane.host !== 'float')
                throw new Error('layout: float attachment requires a floating pane');
...
function applyMoveTab(state, op) {
    ...
    if (from.host !== 'dock')
        throw new Error('layout: moveTab source must be docked; use unfloat');
    const to = getPane(state, op.toPaneId);
    if (to.host !== 'dock')
        throw new Error('layout: moveTab target must be docked');
    if (to.id === from.id)
        throw new Error('layout: moveTab across one pane; use reorderTab');
...
function applyResize(state, op) {
    ...
    if (op.sizes.length !== split.children.length)
        throw new Error('layout: resize sizes do not match the split');
    if (op.sizes.some(size => !(size > 0)))
        throw new Error('layout: resize sizes must all be above zero');
    return {
...
function applyFloat(state, op) {
    ...
    const from = findTabPane(state, op.tabId);
    if (from.host !== 'dock')
        throw new Error('layout: float requires a docked tab');
    assertFreeNode(state, op.newPaneId);
    const index = from.tabs.indexOf(op.tabId);
...
function applyUnfloat(state, op) {
    ...
    const to = getPane(state, op.toPaneId);
    if (to.host !== 'dock')
        throw new Error('layout: unfloat target must be docked');
...
function applyResizeFloat(state, op) {
    ...
    if (!(op.rect.width > 0) || !(op.rect.height > 0))
        throw new Error('layout: float size must be above zero');
    return {
...
function applyRestoreFocus(state, op) {
    ...
    for (const paneId of op.floats) {
        ...
        if (pane.host !== 'float')
            throw new Error(`layout: restoreFocus lists docked pane ${paneId} as floating`);
    }
    let next = state;
 ...
 */
export function replay(state, ops) {
