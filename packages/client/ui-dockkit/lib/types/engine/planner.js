import { canSplit, clampSizes, FLOAT_DEFAULT_SIZE, zoneSplit } from "./constraints.js";
import { applyOp } from "./operations.js";
import { dockPaneIds, findTabPane, firstDockPaneId, getNode, getPane, getTab } from "./tree.js";
 ...
 */
export function findPaneContentTab(state, paneId, contentId, kind) {
    for (const tabId of getPane(state, paneId).tabs) {
        const tab = state.tabs[tabId];
        if (tab?.contentId === contentId && (kind === undefined || tab.kind === kind))
            return tabId;
    }
    return undefined;
}
 ...
 */
export function findContentTab(state, contentId, kind) {
 ...
 */
export function activeDockPaneId(state) {
 ...
 */
export function planSetExpanded(state, expanded) {
 ...
 * @returns the operation, or none when the value is already current.
 */
export function planSetMode(state, mode) {
 ...
 */
export function planSplitPane(state, mint, paneId, makePaneTab) {
    if (!canSplit(state))
        return NOTHING;
    const target = paneId ?? activeDockPaneId(state);
    if (getPane(state, target).host !== 'dock')
        return NOTHING;
    ...
    if (seed !== undefined)
        ops.push({ type: 'openTab', paneId: newPaneId, tab: seed, index: 0 });
    return ops;
}
/**
 ...
 */
export function planAddTab(state, mint, paneId, makeTab) {
    if (makeTab === undefined)
        return NOTHING;
 ...
 */
export function planOpenContent(state, mint, input) {
    const existing = input.revealIfOpened === false
        ? undefined
        : findContentTab(state, input.contentId, input.kind);
    if (existing !== undefined)
        return { ops: [{ type: 'focusTab', tabId: existing }], tabId: existing };
 ...
 */
export function planDuplicateTab(state, mint, tabId) {
 ...
 */
export function planPlaceTab(state, tabId, toPaneId, index) {
    const source = findTabPane(state, tabId);
    if (getPane(state, toPaneId).host !== 'dock')
        return NOTHING;
 ...
 */
export function planDropTab(state, mint, tabId, targetPaneId, zone, makeTab) {
    const source = findTabPane(state, tabId);
 ...
 */
export function planFloatTab(state, mint, tabId, rect) {
 ...
 */
export function planUnfloatPane(state, paneId, toPaneId) {
 ...
 */
export function planResizeSplit(splitId, sizes, minimum) {
 ...
 */
export function planSettle(state, mint, makeTab) {
    const ops = [];
    let current = state;
    for (;;) {
