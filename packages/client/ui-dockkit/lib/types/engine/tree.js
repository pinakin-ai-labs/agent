 ...
 */
export function assertNever(value, what) {
...
export function getNode(state, id) {
    ...
    if (node === undefined)
        throw new Error(`layout: unknown node ${id}`);
    return node;
}
/**
 * Read a pane.
 ...
 */
export function getPane(state, id) {
    const node = getNode(state, id);
    if (node.kind !== 'pane')
        throw new Error(`layout: ${id} is not a pane`);
    return node;
}
/**
 * Read a split.
 ...
 */
export function getSplit(state, id) {
    const node = getNode(state, id);
    if (node.kind !== 'split')
        throw new Error(`layout: ${id} is not a split`);
    return node;
}
/**
 * Read a tab record.
 ...
 */
export function getTab(state, id) {
    const tab = state.tabs[id];
    if (tab === undefined)
        throw new Error(`layout: unknown tab ${id}`);
    return tab;
}
/**
 ...
 */
export function floatRect(pane) {
    if (pane.host !== 'float' || pane.rect === undefined)
        throw new Error(`layout: ${pane.id} is not floating`);
    return pane.rect;
}
/**
 ...
 */
export function floatIndex(state, id) {
    const index = state.floats.indexOf(id);
    if (index < 0)
        throw new Error(`layout: floating pane ${id} is not in the z order`);
    return index;
}
/**
 ...
 */
export function onlyTabId(pane) {
    const tabId = pane.tabs[0];
    if (tabId === undefined || pane.tabs.length !== 1)
        throw new Error(`layout: ${pane.id} does not hold exactly one tab`);
    return tabId;
}
/**
 ...
 */
export function findParent(state, id) {
    for (const node of Object.values(state.nodes)) {
        if (node.kind === 'split' && node.children.includes(id))
            return node;
    }
    return undefined;
}
 ...
 */
export function findTabPane(state, tabId) {
    for (const node of Object.values(state.nodes)) {
        if (node.kind === 'pane' && node.tabs.includes(tabId))
            return node;
    }
...
export function normalizeSizes(sizes) {
    ...
    if (!(total > 0))
        throw new Error('layout: sizes must sum above zero');
 ...
 */
export function withNodes(state, updates) {
    const nodes = {};
 ...
 */
export function withTabs(state, updates) {
    const tabs = {};
 ...
 */
export function neighbourTabId(tabs, removedIndex) {
 ...
 */
export function paneWithTabs(pane, tabs, activeTabId) {
 ...
 */
export function replaceInParent(state, targetId, replacementId) {
    const parent = findParent(state, targetId);
    if (parent === undefined) {
        if (state.rootId !== targetId)
            throw new Error(`layout: ${targetId} is neither rooted nor parented`);
...
function descend(state, choose) {
    ...
    while (node.kind === 'split') {
        ...
        if (next === undefined)
            throw new Error(`layout: split ${node.id} has no children`);
 ...
 */
export function firstDockPaneId(state) {
 ...
 */
export function topRightPaneId(state) {
