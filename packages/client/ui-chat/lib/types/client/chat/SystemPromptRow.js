 ...
 */
export function SystemPromptRow({ text, update = false, t }) {
    const [open, setOpen] = useState(false);
    return (_jsx(DisclosureRow, { className: css.root, icon: _jsx(IconBrowseOutline16, { size: 14 }), chevronClassName: css.chevron, title: t(update ? 'message.systemPromptUpdate' : 'message.systemPrompt'), open: open, expandable: true, expandOnRowClick: true, onToggle: () => { setOpen(value => !value); }, children: _jsx("div", { className: css.body, "data-system-prompt-body": true, children: _jsx(OpaqueBody, { content: [{ type: 'text', text }], source: null, t: t }) }) }));
}
/** System-prompt keyed Chat renderer. */
export const SystemPromptNodeView = memo(function SystemPromptNodeView({ node, t, }) {
    return _jsx(SystemPromptRow, { text: node.data.text, update: node.data.update === true, t: t });
});
