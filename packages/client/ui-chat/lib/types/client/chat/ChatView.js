...
function TurnStatus({ startTime, t }) {
    ...
    return (_jsxs("div", { className: css.turnStatus, role: "status", "aria-live": "polite", children: [t('chat.deepDiving'), showClock && (_jsx("span", { className: css.turnStatusClock, "aria-hidden": true, children: formatRunDuration(elapsedMs, t) }))] }));
}
const ChatNodeList = memo(function ChatNodeList({ order, ...seatProps }) {
    return order.map(nodeKey => (_jsx(ChatNodeSeat, { nodeKey: nodeKey, ...seatProps }, nodeKey)));
});
/**
 * The chat view slot entry: pure component over the composed props; each
 * ordered business Node crosses the keyed renderer seat.
 */
export function ChatView({ useSession, useChat, useChatNode, useChatNodeProcess, useSessions, useStore, actions, renderSlot, sessionId, openFile, openSkill, loadOlder, loadThrough, loadImage, openView, chatScroll, forkAt, fileMentions, useTranscriptView, useProjection, t, }) {
    ...
    useEffect(() => {
        ...
        if (realizePendingJump(local, el, true))
            return;
        ...
        for (const row of local.querySelectorAll('[data-chat-turn]:not([hidden])')) {
            const turn = Number(row.dataset.chatTurn);
            if (!Number.isSafeInteger(turn) || turn < pending.turn)
                continue;
            landOnRowRef.current(local, el, row, turn);
            break;
        }
    ...
    useEffect(() => {
        if (!loadingOlder && pendingJumpRef.current !== null)
    ...
    return (_jsxs("div", { className: css.root, children: [_jsxs("div", { ref: listRef, className: css.scroll, children: [_jsx(TurnNavigator, { items: railItems, activeTurn: activeTurn, busyTurn: busyJumpTurn, onNavigate: navigateToTurn, t: t }), _jsxs("div", { ref: columnRef, className: css.column, "data-chat-flow": "", children: [openState === 'loading' && _jsx("div", { className: css.hint, children: t('chat.loadingHistory') }), openState === 'error' && openError !== null && (_jsx("div", { className: css.openError, children: t('chat.loadError', { message: openError.message, code: openError.code }) })), hasMore && (_jsx("div", { className: css.older, children: _jsx("button", { type: "button", disabled: loadingOlder, onClick: loadOlderAnchored, children: loadingOlder ? t('loading') : t('chat.loadOlder') }) })), _jsx(ChatNodeList, { order: order, useChatNode: useChatNode, useChatNodeProcess: useChatNodeProcess, historyIncomplete: hasMore, compactTranscript: compactTranscript, useStore: useStore, actions: actions, cwd: cwd, openFile: requestOpenFile, openSkill: openSkill, inspectCall: inspectCall, forkAt: forkAt, loadImage: loadImage, renderMessageImages: renderMessageImages, fileMentions: fileMentions, renderSlot: renderSlot, t: t }), running && _jsx(TurnStatus, { startTime: runningTurnStart, t: t }), pendingSteering.map(item => (_jsx(PendingSteeringBubble, { content: item.content, renderMessageImages: renderMessageImages, t: t }, item.id))), visibleSubmissions.map(submission => (_jsx(PendingSubmissionBubble, { submission: submission, renderMessageImages: renderMessageImages, t: t }, submission.requestId)))] }), !atBottom && (_jsx("div", { className: css.toBottomSlot, children: _jsx("button", { type: "button", className: css.toBottom, "aria-label": t('chat.toBottom'), onClick: () => {
                                const local = listRef.current;
                                /* v8 ignore next -- ref-null guard: the button only renders alongside the mounted list. */
                                if (local !== null)
                                    toBottom(scrollerOf(local));
                            }, children: _jsx(IconChevronDownOutline14, {}) }) }))] }), fileOpenError !== null && (_jsx(FileOpenErrorDialog, { message: fileOpenError.message, busy: fileOpenBusy, onClose: closeFileOpenError, onRetry: () => { requestOpenFile(fileOpenError.path); }, t: t }))] }));
}
