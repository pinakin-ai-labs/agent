...
function validSearchCall(block) {
    ...
    if (typeof pattern !== 'string')
        return null;
    if (call.name === 'grep' && pattern === '')
        return null;
    if (call.name === 'glob' && pattern.trim() === '')
        return null;
    ...
    if (call.name === 'grep') {
        ...
        if (include !== undefined && (typeof include !== 'string' || !validInclude(include)))
            return null;
    }
    return call.name;
}
function validInclude(include) {
    if (include.trim() === '' || include.startsWith('!'))
        return false;
    let braceDepth = 0;
...
export function searchCardModel(block) {
    ...
    if (typeof meta.truncated !== 'boolean')
        return null;
    if (typeof meta.total !== 'number' || !Number.isInteger(meta.total) || meta.total < 0)
        return null;
    ...
    if (meta.shape !== 'paths' || !Array.isArray(meta.paths))
        return null;
    if (!meta.paths.every((path) => typeof path === 'string'))
        return null;
