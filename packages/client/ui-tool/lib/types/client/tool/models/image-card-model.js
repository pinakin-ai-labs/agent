...
function imageReferences(content) {
    ...
    for (const part of content) {
        ...
        if (typeof attachmentId !== 'string' || attachmentId === '')
            return null;
        if (typeof mediaType !== 'string' || !isImageMediaType(mediaType))
            return null;
        ...
        if (originalDimensions !== undefined) {
            ...
            if (!positiveInteger(inputWidth) || !positiveInteger(inputHeight))
                return null;
...
function imageTexts(content) {
    ...
    for (const part of content) {
        if (part.type !== 'text' || typeof part.text !== 'string')
            continue;
        if (IMAGE_ENVELOPE.test(part.text))
            sawEnvelope = true;
 ...
 */
export function imageCardModel(block, sessionCwd, home) {
    ...
    if (!fullyRendered(block.content))
        return null;
...
//# sourceMappingURL=image-card-model.js.map