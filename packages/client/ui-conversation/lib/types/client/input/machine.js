...
export class SubmitMachine {
    ...
    onSubmitSettled(ev) {
        ...
        if (ev.ok) {
            this.phase = 'plain';
            this.claim = undefined;
            const effects = [{ type: 'commit-draft', retainSuffixOf: flight.attempt.draftSnapshot }];
    ...
    onSinkSettled(ev) {
        if (!this.detached.delete(ev.attempt.seq))
            return [];
        const text = ev.message ?? ev.outcome?.text;
        if (text === undefined)
            return [];
    ...
    onRelease() {
        ...
        for (const controller of this.detached.values())
            controller.abort();
        this.detached.clear();
        this.phase = 'plain';
        this.claim = undefined;
