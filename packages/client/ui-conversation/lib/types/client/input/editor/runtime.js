...
import { registerReferenceActivation } from "./reference-activation.js";
import { ReferenceChipNode, $createReferenceChipNode } from "./chip-node.js";
import { refreshClaimDecoration, registerClaimDecoration } from "./claim-decor.js";
import { registerTextRefDecoration, rescanTextRefs, TextRefNode } from "./text-ref.js";
import { $composerLayout, $projectComposer, detectOffsetOfClipboardOffset } from "./projection.js";
import { $replaceDetectSpanWithNodes, $replaceDetectSpanWithText } from "./span-map.js";
/**
...
export class DraftEditorRuntime {
    ...
    editor;
    projected = { detectText: '', clipboardText: '', occurrences: [], selection: null, caret: null };
     ...
     */
    register() {
        const unregister = mergeRegister(registerPlainText(this.editor), registerReferenceActivation(this.editor, (source, reference) => this.deps.openReference(source, reference)), registerHistory(this.editor, createEmptyHistoryState(), HISTORY_MERGE_DELAY_MS), this.editor.registerUpdateListener(() => { this.deps.onUpdate(); }), registerClaimDecoration(this.editor, () => this.deps.activeClaimToken()), registerTextRefDecoration(this.editor, () => this.deps.lexicon(), () => this.deps.activeClaimToken()), () => { this.lexiconOff?.(); });
        return () => {
            unregister();
    ...
    clearCommittedDraft(prefixLength) {
        this.editor.update(() => {
            ...
            if (length !== null) {
                $replaceDetectSpanWithText({ start: 0, end: detectOffsetOfClipboardOffset(layout, length) }, '');
                return;
            }
            const root = $getRoot();
            root.clear();
