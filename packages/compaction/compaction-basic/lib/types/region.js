...
export function selectCompactableRange(session, measurement, retainTokens) {
    ...
    while (keepFromIdx > firstIdx) {
        // oxlint-disable-next-line typescript/no-non-null-assertion
        if (toolPairingBalancedBefore(session, surfaceNodes[keepFromIdx]))
            break;
        keepFromIdx -= 1;
    }
    if (keepFromIdx <= firstIdx)
...
export async function compactSurfaceRegion(dependencies, session, start, end, agent, options, signal) {
    ...
    const assertStable = options.stability === 'whole-surface'
    ...
    try {
        ...
        const summarized = await summarizeCompaction(dependencies, prepared, agent, compactionId, options.sourceCommandId, assertStable, signal);
        if (options.owner === null)
            signal?.throwIfAborted();
