...
export const launchDetachedApp = (command, args, options) => new Promise((resolve, reject) => {
    ...
    child.on('exit', (code, signalName) => {
        if (code === 0)
            ...
            settle(() => { reject(new Error(`launcher exited with code ${String(code)}, signal ${String(signalName)}`)); });
    });
});
/**
 ...
 */
export function resolveInternals(internals) {
 ...
 */
export async function output(command, args, timeoutMs, internals) {
    try {
 ...
 */
export function expandCandidate(template, internals) {
    const unset = [];
...
export function parseRegistryDump(dump) {
    ...
    for (const line of dump.split(/\r?\n/)) {
        ...
        if (value === null || current === undefined)
            continue;
 ...
 */
export async function readWindowsRegistryView(timeoutMs, internals) {
    ...
    for (const root of APP_PATHS_ROOTS) {
        ...
        for (const [key, values] of parseRegistryDump(dump)) {
            ...
            if (!exe.endsWith('.exe') || target === undefined || appPaths.has(exe))
                continue;
            const expanded = expandRegistryValue(target.replace(/^"|"$/g, ''), internals);
            if (expanded !== null)
                appPaths.set(exe, expanded);
        }
    }
...
async function recordLauncher(record, relativeLauncher, internals) {
    ...
    if (record.displayIcon !== undefined) {
        ...
        if (expanded !== null && expanded.toLowerCase().endsWith('.exe') && await isFile(expanded))
            return expanded;
    }
    return null;
}
 ...
 */
export function xdgDataDirectories(internals) {
 ...
 */
export async function findDesktopEntry(desktopId, internals) {
 ...
 */
export function specFor(app, platform) {
...
async function locate(locator, probeTimeoutMs, registry, internals) {
    switch (locator.kind) {
        ...
        case 'xcode': {
            ...
            if (!bundle.endsWith('.app') || !await isDirectory(bundle))
                return null;
            return {
        ...
        case 'app-paths': {
            ...
            if (target === undefined || !await isFile(target))
                return null;
        ...
        case 'install-record': {
            for (const record of (await registry.read()).installRecords) {
                if (!record.displayName.startsWith(locator.displayNamePrefix))
                    continue;
 ...
 */
export async function resolveLaunch(app, probeTimeoutMs, internals = {}) {
 ...
 */
export async function resolveOpenInAppApps(probeTimeoutMs, internals = {}) {
    const resolved = resolveInternals(internals);
    if (resolved.ssh) {
        return new Map();
    }
 ...
 */
export async function launchResolved(resolved, path, watchMs, internals = {}) {
    const completed = resolveInternals(internals);
    const primary = await runLaunch(resolved.launch, path, watchMs, completed);
    if (primary === 'launched' || resolved.fallbackLaunch === undefined)
        return primary;
