...
import { findDesktopEntry, isFile, output, resolveInternals, specFor, xdgDataDirectories, } from "./resolver.js";
/**
...
async function extractBundleIconPng(bundlePath, timeoutMs, internals) {
    ...
    let iconFile = null;
    const plistJson = await output('plutil', ['-convert', 'json', '-o', '-', join(bundlePath, 'Contents', 'Info.plist')], timeoutMs, internals);
    if (plistJson !== null) {
        try {
...
const HICOLOR_SIZES = ['512x512', '256x256', '128x128', '64x64', '48x48', '32x32'];
...
async function readIconFile(path) {
    ...
    if (contentType === null || !await isFile(path))
        return null;
 ...
 */
export async function extractAppIcon(app, resolved, timeoutMs, internals = {}) {
