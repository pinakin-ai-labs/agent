...
const READING = new RegExp('^Time sampled while preparing turn (\\d+), step (\\d+): '
...
function preparationPosition(history, fail) {
    ...
    if (openStep === undefined)
        fail('time-context reading must follow step/start');
    if (requestStarted)
        fail('time-context reading must precede request/header');
...
function validateReading(history, event, fail) {
    ...
    if (match === null)
        fail('time-context message does not match the durable reading format');
    ...
    const sectionValue = Array.isArray(sections) ? sections[0] : undefined;
