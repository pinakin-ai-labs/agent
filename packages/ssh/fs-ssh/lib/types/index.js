...
export class SshFileSystem extends FileSystem {
    ...
    async resolve(path, opts) {
        return await this.call('fs.resolve', { path, cwd: opts?.cwd }, targetSchema, opts?.signal);
    }
    ...
    async stat(target, signal) {
        return await this.call('fs.stat', { target }, infoSchema.nullable(), signal) ?? undefined;
    }
    async lstat(path, opts, signal) {
        return await this.call('fs.lstat', { path, cwd: opts?.cwd }, pathInfoSchema.nullable(), signal) ?? undefined;
    }
    ...
    async streamText(target, signal) {
        ...
        return (async function* () {
            ...
            try {
                ...
                if (!ended)
                    await call('fs.streamClose', { id }, z.null()).catch(() => { });
            }
        })();
    }
    ...
    async writeText(target, content, expected, signal, sandboxPolicy) {
        ...
        return await this.call('fs.write', { target, content, expected, policy }, writeResultSchema, signal);
    }
    async editText(target, edit, expected, signal, sandboxPolicy) {
        const policy = sandboxPolicy ?? this.ctx.sandboxPolicy.resolve();
        return await this.call('fs.edit', { target, edit, expected, policy }, editResultSchema, signal);
    }
