...
class RemoteProcess {
    ...
    constructor(ssh, spec) {
        ...
        for (const stream of [this.inbound, this.out, this.err, this.toControl, this.fromControl, this.control])
            stream?.on('error', () => { });
        const collect = (name, stream, mode) => {
            ...
            this.updateCollection[name] = (snapshot, final) => {
                ...
                if (bytes.length > mode.maxBytes || snapshot.totalBytes < bytes.length)
                    throw new Error('SSH helper returned invalid collected output coordinates');
                if (finalized)
                    return;
                if (snapshot.totalBytes < total)
                    throw new Error('SSH helper rewound collected output');
                finalized = final;
        ...
        this.done = this.started.then(async () => {
            ...
            for (const name of ['stdout', 'stderr']) {
                ...
                if ((snapshot === undefined) !== (finish === undefined))
                    throw new Error('SSH helper returned mismatched output collection modes');
                if (snapshot !== undefined)
                    finish?.(snapshot, true);
            }
            return { exitCode: result.outcome.exitCode, signal: result.outcome.signal };
        }).catch((error) => {
            this.terminate();
    ...
    async start() {
        ...
        const prepared = await this.ssh.request('process.prepare', { ...this.spec, signal: undefined, env: environment(this.spec.env) }, preparedSchema, this.controller.signal);
        this.id = prepared.id;
        try {
                ...
                if (name === 'stdout' || name === 'stderr') {
                    ...
                    if (typeof mode === 'object') {
                        new SshRpcPeer(socket, socket, outputSnapshotFrameLimit(mode.maxBytes), 1, (method, raw) => Promise.resolve().then(() => {
                            if (method !== 'snapshot')
                                throw new Error('Unexpected SSH output-stream operation');
    ...
    terminate() {
        if (this.quiescent || this.termination !== undefined)
            return;
        if (!this.committed)
            this.controller.abort(new Error('SSH process terminated before launch acknowledgement'));
    ...
    async drainOutput() {
        ...
        const outputs = ['stdout', 'stderr'].map((name) => {
            if (typeof this.spec.stdio[name] === 'object')
            ...
            return new Promise((resolve) => {
                ...
                disposers.push(() => { for (const event of ['end', 'close', 'error'])
        ...
        try {
            await Promise.race([
                Promise.all(outputs),
                new Promise((resolve) => { timer = setTimeout(resolve, this.spec.graceMs); }),
            ]);
        }
        finally {
            clearTimeout(timer);
...
export class SshSubprocessRuntime extends SubprocessRuntime {
        ...
        ctx.effect(() => async () => {
            ...
            const results = await Promise.allSettled([
                ...
                ...[...this.terminalAllocations].map(allocation => allocation.catch((error) => {
                    if (error instanceof RemoteCleanupError)
                        throw error;
                })),
            ...
            if (errors.length > 0)
                throw new AggregateError(errors, 'SSH process cleanup could not be confirmed');
        });
    }
    ...
    async createTerminal(spec, signal) {
        ...
        try {
            ...
            signal.throwIfAborted();
            const done = ssh.request('process.done', { id }, doneSchema, undefined, true).then(result => ({ exitCode: result.outcome.exitCode, signal: result.outcome.signal }));
            void done.catch(() => { });
            let closing;
            const abort = () => { void handle.terminate().catch(() => { void ssh.dispose().catch(() => { }); }); };
            const handle = {
                ...
                signalForeground: (signal) => ssh.request('terminal.signal', { id, value: signal }, z.number().int().positive()),
                terminate: () => {
