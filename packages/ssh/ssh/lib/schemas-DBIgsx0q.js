...
const processIdSchema = z.uuid().transform((value) => value);
/** Admit a text iterator identity from the private helper protocol. */
const textStreamIdSchema = z.uuid().transform((value) => value);
/** A remote POSIX absolute path; spelling is preserved until remote canonicalization. */
const remotePath = z.string().min(1).refine((value) => value.startsWith("/") && !value.includes("\0"), "expected an absolute POSIX path");
/** Filesystem identity returned by the remote filesystem provider. */
const targetSchema = z.object({
	targetKey: remotePath,
...
const pathInfoSchema = infoSchema.extend({ type: z.enum([
	"file",
	"directory",
	"symlink",
	"other"
...
const intentSchema = z.discriminatedUnion("kind", [z.object({ kind: z.literal("createIfAbsent") }).strict(), z.object({
	kind: z.literal("replaceIfVersion"),
	version: z.string()
}).strict()]);
/** Literal text edit. */
...
const spawnSchema = z.object({
	...
	graceMs: z.number().int().positive().max(2147483647),
	stdio: z.object({
		stdin: z.union([
			z.literal("ignore"),
			z.literal("pipe"),
			z.object({ data: z.string() }).strict()
		]),
		stdout: z.union([
			z.literal("pipe"),
			z.literal("inherit"),
...
}).strict().refine((value) => value.stdio === void 0 !== (value.terminal === void 0), "select ordinary or terminal execution");
...
const streamEndpointSchema = z.object({
	path: remotePath,
...
const preparedSchema = z.object({
	id: processIdSchema,
	streams: z.partialRecord(z.enum([
		"stdin",
		"stdout",
		"stderr",
		"control",
...
*/
function outputSnapshotFrameLimit(maxBytes) {
...
const doneSchema = z.object({
	...
	spills: z.object({
		stdout: remotePath.optional(),
		stderr: remotePath.optional()
	}).strict(),
	collected: z.object({
...
const foregroundSchema = z.object({
	...
	inputWaiting: z.boolean()
}).strict().nullable();
//#endregion
export { writeResultSchema as S, remotePath as _, environmentSchema as a, targetSchema as b, infoSchema as c, outputSnapshotFrameLimit as d, outputSnapshotSchema as f, processIdSchema as g, preparedSchema as h, entriesSchema as i, intentSchema as l, policySchema as m, editResultSchema as n, foregroundSchema as o, pathInfoSchema as p, editSchema as r, helloSchema as s, doneSchema as t, outcomeSchema as u, spawnSchema as v, textStreamIdSchema as x, streamEndpointSchema as y };
