...
export class SshRpcPeer extends EventEmitter {
    ...
    constructor(input, output, maxFrameBytes, maxPending, handler) {
        ...
        void this.readFrames().catch((error) => { this.close(operationError(error)); });
    }
    /**
    ...
    async request(method, params, schema, signal) {
        ...
        if (this.atCapacity(kind, this.pending.values()))
            throw new Error('SSH helper pending request limit reached');
        const id = randomUUID();
        ...
        try {
            void this.send({ type: 'request', id, method, params }).catch((error) => {
     ...
     */
    close(error = new Error('SSH connection lost; remote operation outcome and cleanup are unknown')) {
        if (this.failure !== undefined)
            return;
        this.failure = error;
    ...
    async send(frame) {
        ...
        this.writeTail = write.catch((error) => { this.close(operationError(error)); });
            ...
            while (offset < chunk.length) {
                if (payload === undefined) {
                    ...
                    if (size === 0 || size > this.maxFrameBytes)
                        throw new Error('SSH helper sent an invalid frame length');
    ...
    receive(frame) {
        if (frame.type === 'result' || frame.type === 'error') {
            ...
            if (pending === undefined)
                return; // A cancelled request can still complete remotely.
            this.pending.delete(frame.id);
            if (frame.type === 'result')
                pending.resolve(frame.value);
            else
                pending.reject(new RemoteOperationError(frame.error.message, frame.error.code));
            return;
        }
        ...
        void this.handler(frame.method, frame.params, controller.signal).then(value => this.send({ type: 'result', id: frame.id, value: value ?? null }), (error) => {
        ...
        }).catch((error) => { this.close(operationError(error)); })
