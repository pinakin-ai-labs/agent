...
import { runSshHelper } from "./helper.js";
const controller = new AbortController();
const stop = () => { controller.abort(new Error('SSH helper process terminated')); };
for (const signal of ['SIGTERM', 'SIGHUP', 'SIGINT'])
    process.once(signal, stop);
try {
    if (process.argv.length !== 2)
        throw new Error('SSH helper accepts no command arguments');
