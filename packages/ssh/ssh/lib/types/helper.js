...
import { z } from 'zod';
import { SshRpcPeer, RemoteOperationError, SSH_MAX_PROCESS_HANDLES, SSH_MAX_TEXT_STREAMS, SSH_PROTOCOL_VERSION } from "./protocol.js";
import { RemoteProcesses } from "./helper-processes.js";
import { editSchema, environmentSchema, intentSchema, policySchema, processIdSchema, remotePath, targetSchema, textStreamIdSchema } from "./schemas.js";
...
async function services() {
    ...
    return { ctx, close: async () => { for (const fiber of fibers.reverse())
            await fiber.dispose(); } };
}
/**
 ...
 */
export async function runSshHelper(transport) {
    if (process.platform !== 'linux' && process.platform !== 'darwin')
        throw new Error('SSH helper requires a POSIX host');
    ...
    const close = () => {
        cleanup ??= (async () => {
            ...
            for (const record of iterators.values())
                record.controller.abort(lifetime.signal.reason);
    ...
    const policy = async (raw, signal) => {
        ...
        return { ...parsed, workspaceRoot: ctx.fs.processPath(target) };
    };
    ...
    const peer = new SshRpcPeer(transport.input, transport.output, MAX_FRAME_BYTES, 128, async (method, raw, requestSignal) => {
        ...
        if (method === 'hello') {
            if (initialized)
                throw new Error('SSH helper handshake already completed');
        ...
        if (!initialized || cleanup !== undefined)
            throw new Error('SSH helper is not accepting operations');
        if (method === 'heartbeat') {
            object.parse(raw);
            touchLease();
            return null;
        ...
        if (method === 'process.prepare')
            return processes.prepare(raw);
        if (method === 'process.start')
            return processes.start(processIdRequest.parse(raw).id, signal);
        if (method === 'process.done')
            return processes.done(processIdRequest.parse(raw).id);
        if (method === 'process.wait')
            return processes.wait(processIdRequest.parse(raw).id, signal);
        ...
        if (method === 'executable') {
            ...
            const env = input.env === undefined ? undefined : Object.fromEntries(Object.entries(input.env).filter((entry) => entry[1] !== null));
            try {
        ...
        if (method === 'sandbox') {
            ...
            if (resolved.mode === 'danger-full-access')
                throw new Error('Unconfined argv does not need a sandbox wrapper');
        ...
        if (method === 'fs.stat' || method === 'fs.list' || method === 'fs.readText' || method === 'fs.stream') {
            ...
            if (method === 'fs.stat')
                return await ctx.fs.stat(target, signal) ?? null;
            if (method === 'fs.list')
                return ctx.fs.listDir(target, signal);
            if (method === 'fs.stream') {
                if (iterators.size >= SSH_MAX_TEXT_STREAMS)
                    throw new Error('SSH text stream limit reached');
            ...
            for await (const chunk of stream) {
                ...
                if (bytes > MAX_TEXT_BYTES)
                    throw new FsError('SSH whole-text transfer exceeds its bounded frame budget; use streaming', 'FS_TOO_LARGE');
                text += chunk;
            }
            return text;
        }
        if (method === 'fs.next' || method === 'fs.streamClose') {
            ...
            if (record === undefined)
                throw new Error('Unknown SSH text stream');
        ...
        if (method === 'fs.write' || method === 'fs.edit') {
            ...
            const resolved = await policy(input.policy, signal);
            if (method === 'fs.write')
                return ctx.fs.writeText(target, z.string().parse(input.content), input.expected === undefined ? undefined : intentSchema.parse(input.expected), signal, resolved);
            return ctx.fs.editText(target, editSchema.parse(input.edit), input.expected === undefined
                ? undefined : z.object({ version: z.string() }).strict().parse(input.expected), signal, resolved);
        }
    ...
    const onAbort = () => { peer.close(new Error('SSH helper transport was terminated')); };
