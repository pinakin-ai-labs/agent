...
function requireAgent(agent) {
    if (agent === undefined)
        throw new Error('terminal tools require an initiating agent');
    return agent;
}
        ...
        async execute(args, exec) {
            ...
            if (args.run_in_background === true) {
                if (!enableRunInBackground)
                    throw new Error('background terminal sends are disabled by tool-terminal configuration');
            ...
            if (exec.signal.aborted)
                throw new Error('terminal send aborted');
