 ...
 */
export async function loadPluginModules(plan) {
...
async function importClient(name) {
    let namespace;
    try {
        namespace = await import(__rewriteRelativeImportExtension(/* @vite-ignore */ `${name}/client`));
    }
    catch (error) {
 ...
 */
export function createInProcessModules(graph, modules) {
    const target = {
        mode: 'queue',
        pendingQueue: [],
    ...
    for (const [id, namespace] of modules) {
        if (id === MODULES_PACKAGE)
            continue;
        target.pendingQueue.push({ id, factory: () => namespace });
    }
