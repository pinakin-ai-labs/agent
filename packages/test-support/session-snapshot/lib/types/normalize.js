 ...
 */
import { decodeSeqRanges, } from '@deepseek-ai/dsh-session';
import { prepareSessionSnapshotFixtureForComparison } from '@deepseek-ai/dsh-llm-replay';
import { redactSessionSnapshotIds } from "./identity.js";
...
const LOCAL_SPILL_PATH_RE = new RegExp(String.raw `\{\{cwd\}\}[\\/]+\.spill[\\/]+session-[0-9a-f]{12}[\\/]+[0-9a-f]{12}-([A-Za-z0-9._~-]+?)`
    + String.raw `(?=\. Use read with offset/limit|[\s)"]|\\+"|$)`, 'g');
const SNAPSHOT_SPILL_PATH_RE = new RegExp(String.raw `(?:[A-Za-z]:)?[\\/]+(?:tmp|t)[\\/]+(?:dsh-acp-snap-[0-9a-f]{9}|dsh-acp-snapshot-spill)[\\/]+session-[0-9a-f]{12}[\\/]+[0-9a-f]{12}-([A-Za-z0-9._~-]+?)`
    + String.raw `(?=\. Use read with offset/limit|[\s)"]|\\+"|$)`, 'g');
/**
 ...
 */
export function extractSnapshotSpillPaths(content) {
...
function canonicalizeEmbeddedPaths(value) {
    return value
        .replace(PATH_TAG_RE, (_match, open, path, close) => `${open}${path.replaceAll('\\', '/')}${close}`)
        .replace(ADDITIONAL_INSTRUCTIONS_PATH_RE, (_match, prefix, path) => `${prefix}${path.replaceAll('\\', '/')}`);
}
...
function scrubString(value, ctx, cwdPathMode, identityMode) {
    ...
    if (identityMode === 'legacy') {
        for (const id of ctx.sessionIds)
            out = out.split(id).join(SESSION_ID);
...
function scrubValue(value, ctx, cwdPathMode, identityMode, key) {
    if (typeof value === 'string') {
        if (identityMode === 'legacy' && key === 'messageId')
            return MESSAGE_ID;
    ...
    if (Array.isArray(value))
        return value.map(v => scrubValue(v, ctx, cwdPathMode, identityMode));
    if (value !== null && typeof value === 'object') {
        const out = {};
        for (const [k, v] of Object.entries(value))
            out[k] = scrubValue(v, ctx, cwdPathMode, identityMode, k);
...
function tokenizeFixtureString(value, ctx, basename) {
    ...
    const absoluteCwd = new RegExp(String.raw `(?:[A-Za-z]:)?[\\/](?:[^\\/\s<>"]+[\\/])*${escapeRegExp(basename)}`
        + String.raw `(?=$|[\\/\s<>'"()\[\]{},;:!?=])`, 'g');
...
function tokenizeFixtureValue(value, ctx, basename) {
    if (typeof value === 'string')
        return tokenizeFixtureString(value, ctx, basename);
    if (Array.isArray(value))
        return value.map(item => tokenizeFixtureValue(item, ctx, basename));
    if (value !== null && typeof value === 'object') {
 ...
 */
export function tokenizeSessionFixtureCwd(rawLog) {
    const lines = rawLog.split('\n');
    const firstLine = lines.find(line => line.trim().length > 0);
    const header = firstLine === undefined ? undefined : JSON.parse(firstLine);
 ...
 */
export function normalizeStdout(rawStdout, ctx, options = {}) {
 ...
 */
export function normalizeSessionLog(rawLog, ctx, options = {}) {
    const cwdPathMode = options.cwdPathMode ?? 'canonical';
    const identityMode = options.identityMode ?? 'legacy';
    const lines = rawLog.split('\n').filter(line => line.trim().length > 0);
    const records = lines.map((line) => {
        ...
        if (record.type === 'session') {
            ...
            if (data !== null && typeof data === 'object' && Array.isArray(data.dt)) {
                data.dt = data.dt.map(() => 0);
            }
        }
            ...
            if (Array.isArray(stream)) {
                for (const member of stream) {
                    if (member === null || typeof member !== 'object')
                        continue;
                    const timed = member;
                    if (typeof timed.time === 'number')
                        timed.time = 0;
                    if (typeof timed.time0 === 'number')
                        timed.time0 = 0;
                    if (Array.isArray(timed.dt))
                        timed.dt = timed.dt.map(() => 0);
                }
            }
        }
 ...
 */
export function normalizeSessionSnapshot(rawLog, ctx, options = {}) {
 ...
 */
export function normalizeSessionSnapshots(rawLogs, ctx, options = {}) {
    ...
    return redactSessionSnapshotIds(comparableLogs).map(log => projectSessionSnapshot(scrubSessionSnapshot(normalizeSessionLog(log, { ...ctx, sessionIds: [] }, { ...options, identityMode: 'preserve' }))));
}
/**
 ...
 */
export function normalizeSessionFormatMetadata(rawLog) {
    return rawLog.split('\n').map((line) => {
        if (line.trim().length === 0)
            return line;
        const record = JSON.parse(line);
 ...
 */
export function scrubSystemPrompts(rawLog) {
 ...
 */
export function scrubToolSchemas(rawLog) {
 ...
 */
export function scrubModelRequestBulk(rawLog) {
 ...
 */
export function scrubSessionSnapshot(rawLog) {
...
function normalizeFeedbackClocks(record) {
    if (record.type !== 'feedback/message-put' || record.data === null || typeof record.data !== 'object')
        return;
    const item = record.data.item;
    if (item === null || typeof item !== 'object')
        return;
    const clocks = item;
    if ('createdAt' in clocks)
        clocks.createdAt = 0;
    if ('updatedAt' in clocks)
        clocks.updatedAt = 0;
}
...
function scrubModelRequestContent(rawLog, options) {
    ...
    const out = lines.map((line) => {
        ...
        if (options.tools === true && record.type === 'request/header') {
            ...
            if (header === null || typeof header !== 'object' || !('tools' in header))
                return line;
            header.tools = TOOLS;
            return JSON.stringify(record);
        }
