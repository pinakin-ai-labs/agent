...
import { client as createAcpClientApp, methods, ndJsonStream, } from '@agentclientprotocol/sdk';
...
export function launchAcpTestAgent(options) {
    ...
    const clientApp = createAcpClientApp({ name: 'deepseek-harness-acp-test-client' })
        ...
        .onRequest(methods.client.session.requestPermission, ({ params }) => (trackClientCallback(() => requestPermission(params))));
 ...
 */
export function materializeProfilePatch(source, cwd, profile, targetDir, index) {
    ...
    const resolveName = (value) => {
        ...
        if (packageName !== undefined)
            linkProfilePackage(source, cwd, profile, packageName);
    ...
    for (const patch of patches) {
        if (typeof patch.name === 'string')
            patch.name = resolveName(patch.name);
