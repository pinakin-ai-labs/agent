 ...
 */
export function spawnPipedProcess(api, options) {
    const owned = new Set();
    let startupInfo;
    let processInfo;
    try {
        ...
        const created = createRestrictedProcess(api, options, buildCommandLine(options.command, options.args), 0, startupInfo, processInfo);
        if (created === 0) {
