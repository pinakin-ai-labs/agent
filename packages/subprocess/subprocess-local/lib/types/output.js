...
process.once('exit', () => {
    if (defaultSpillDir === undefined)
        return;
    try {
        rmdirSync(defaultSpillDir);
    }
    catch { /* best-effort: ENOENT/ENOTEMPTY/EBUSY/EPERM must not change the exit code. */ }
});
/**
 ...
 */
export function prepareManagedProcessBinding(internals = {}) {
