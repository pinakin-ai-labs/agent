...
import { chmodSync, existsSync, lstatSync, mkdtempSync, readFileSync, rmdirSync, unlinkSync, writeFileSync, } from 'node:fs';
...
function isSerializedRunnerError(value) {
    if (!isRecord(value) || !hasExactKeys(value, ['name', 'message'], ['code', 'syscall', 'path']))
        return false;
 ...
 */
export function createLinuxLaunchFiles(request) {
 ...
 */
export function linuxLaunchFilesFromLocator(requestPath) {
 ...
 */
export function consumeLinuxLaunchRequest(requestPath) {
 ...
 */
export function writeLinuxStartupError(files, error) {
 ...
 */
export function readLinuxStartupError(path) {
    if (!existsSync(path))
        return undefined;
    const value = JSON.parse(readFileSync(path, 'utf8'));
    if (!isRecord(value))
        throw new Error('subprocess runner emitted an invalid startup error');
 ...
 */
export function parseWindowsStartRequest(value) {
 ...
 */
export function isWindowsTerminateRequest(value) {
 ...
 */
export function parseWindowsRunnerResult(value) {
 ...
 */
export function serializeRunnerError(error) {
 ...
 */
export function deserializeRunnerError(serialized) {
 ...
 */
export function cleanupLinuxLaunchFiles(files) {
    try {
...
//# sourceMappingURL=runner-protocol.js.map