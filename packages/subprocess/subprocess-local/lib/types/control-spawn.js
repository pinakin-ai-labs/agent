 ...
 */
export function controlPipe(child, control) {
    // Node's type declaration names only the first five descriptor slots.
    const streams = child.stdio;
    return control === 'pipe' ? streams[SUBPROCESS_CONTROL_FD] : undefined;
}
/**
 ...
 */
export function controlEnvironment(env, control) {
    ...
    if (control === 'pipe')
        Object.assign(env, { [SUBPROCESS_CONTROL_ENV]: 'pipe' });
    return env;
}
//# sourceMappingURL=control-spawn.js.map