...
import { controlEnvironment } from "./control-spawn.js";
...
export const SUBPROCESS_RUNNER_ENV = 'DSH_SUBPROCESS_RUNNER';
/** Sentinel used by the packaged bootstrap for the Windows IPC runner. */
export const WINDOWS_RUNNER_SELECTION = 'windows';
 ...
 */
export function spawnRunnerInvocation() {
    if ('pkg' in process)
 ...
 */
export function runnerInvocationAvailable(invocation = spawnRunnerInvocation()) {
    try {
        if (isAbsolute(invocation[0]))
            accessSync(invocation[0], fsConstants.X_OK);
 ...
 */
export function runnerEnvironment(selection, invocation) {
    const entry = invocation?.at(-1);
    const env = childEnv();
 ...
 */
export function consumeRunnerSelection(env = process.env) {
 ...
 */
export function parseRunnerTargetArgv(argv) {
 ...
 */
export function runnerStdio(spec, ipc, stdinCarrier = 'pipe') {
    const targetStdio = [
    ...
    if (!ipc) {
        if (spec.stdio.control === 'pipe') {
            while (targetStdio.length < SUBPROCESS_CONTROL_FD)
                targetStdio.push('ignore');
...
function windowsFileNameStart(command) {
    ...
    while (start > 0 && !/[\\/:]/u.test(command.charAt(start - 1)))
        start -= 1;
    return start;
}
function windowsSearchPathJoin(directory, name, cwd) {
    ...
    const append = (base, part) => {
        if (base.length === 0 || part.length === 0)
            return base + part;
 ...
 */
export function resolveWindowsExecutable(command, cwd, env, exists = executableCandidateExists, currentEnv = process.env) {
...
function validateNoNullByte(property, value, argument = false) {
    if (value.includes('\0'))
        throwNullByteError(property, value, argument);
}
/**
 ...
 */
export function targetEnvironment(spec) {
    ...
    const env = Object.fromEntries(Object.entries(childEnv(spec.env)).filter((entry) => entry[1] !== undefined));
