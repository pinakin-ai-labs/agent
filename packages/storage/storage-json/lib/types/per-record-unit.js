...
export async function openPerRecordUnit(descriptor, root, onClose) {
...
async function loadPerRecordState(descriptor, dir) {
    ...
    if (!hasNewDocuments)
        await bootstrapLegacyUnit(descriptor, dir, state);
    return state;
}
...
async function bootstrapLegacyUnit(descriptor, dir, state) {
    ...
    if (document.unit?.name !== descriptor.name)
        return;
    const stamped = document.unit.version;
    if (typeof stamped !== 'number' || !acceptedStamps(descriptor).includes(stamped))
        return;
 ...
 */
export class PerRecordJsonUnit {
    descriptor;
    dir;
    onClose;
    closed = false;
...
//# sourceMappingURL=per-record-unit.js.map