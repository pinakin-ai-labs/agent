...
export class FileSystemSkillProvider {
    ...
    async roots(cwd) {
        ...
        if (this.includeDefaultRoots && cwd !== undefined) {
            ...
            roots.push({ path: join(projectRoot, '.dsh/skills'), source: 'project-dsh', rank: PROJECT_DSH_RANK, projectRoot }, { path: join(projectRoot, '.agents/skills'), source: 'project-agents', rank: PROJECT_AGENTS_RANK, projectRoot });
        }
        roots.push(...this.customSkillDirs.map(path => ({ path, source: 'custom', rank: CUSTOM_RANK })));
        if (this.includeDefaultRoots) {
            roots.push({ path: join(this.dshHome, 'skills'), source: 'user-dsh', rank: USER_DSH_RANK, skipSystem: true }, { path: join(this.agentsHome, 'skills'), source: 'user-agents', rank: USER_AGENTS_RANK });
        }
...
class SkillWatchManager {
    ...
    async replaceWatcher(state) {
        ...
        if (previous !== undefined)
            await this.closeWatcher(previous);
    ...
    async openRootWatcher(state, mode) {
        ...
        const onAbort = () => { readiness.reject(signal.reason); };
...
async function resolveRootWatchMode(root, followSymlinks) {
    ...
    while (true) {
        ...
        if (parent === candidate)
            return { kind: 'ancestor', anchor: candidate, nextPath: root };
        candidate = parent;
    }
}
...
async function readSkillTextFromFileSystem(ctx, fs, path, signal) {
    ...
    try {
        ...
        if (isAbsentSkillPathError(error))
            return undefined;
        if (!hasErrorCode(error, 'FS_NOT_TEXT'))
            throw error;
