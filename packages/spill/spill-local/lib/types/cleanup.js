...
import { DEFAULT_ROOT_PREFIX, isErrno } from "./store.js";
/**
...
async function hasProtectedAncestors(path) {
    ...
    for (;;) {
        ...
        if (writableByOthers && childStats.uid !== currentUid)
            return false;
        child = parent;
        childStats = stats;
    }
...
async function resolveRoot(path, allowSymlink, warn) {
    ...
    try {
        ...
        if (!isErrno(error, 'ENOENT'))
            warnSafely(warn, `spill-local: failed to inspect root ${path}: ${String(error)}`);
        return undefined;
        /* v8 ignore stop */
    }
    ...
    try {
        ...
        if (!isErrno(error, 'ENOENT'))
            warnSafely(warn, `spill-local: failed to resolve root ${path}: ${String(error)}`);
        return undefined;
        /* v8 ignore stop */
    }
    ...
    try {
        ...
        if (!isErrno(error, 'ENOENT'))
            warnSafely(warn, `spill-local: failed to inspect ancestors of root ${canonical}: ${String(error)}`);
        return undefined;
        /* v8 ignore stop */
    }
 ...
 */
export async function sweepSpillRoots(options) {
    ...
    for (const root of roots.values()) {
        ...
        try {
            ...
            if (!isErrno(error, 'ENOENT'))
                warnSafely(warn, `spill-local: failed to read root ${root.path}: ${String(error)}`);
            continue;
            /* v8 ignore stop */
        }
 ...
 */
export async function discoverDefaultRoots(warn, base = tmpdir()) {
 ...
 */
export async function gatherSweepRoots(activeRoot, warn, defaultRootsBase = tmpdir()) {
    ...
    for (const root of discovered)
        roots.set(root.identity, { path: root.path, pruneWhenEmpty: true });
    if (active !== undefined)
        roots.set(active.identity, { path: active.path, pruneWhenEmpty: false });
    return [...roots.values()];
}
