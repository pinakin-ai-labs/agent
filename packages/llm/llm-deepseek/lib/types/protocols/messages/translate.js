...
import { object, replayState } from "./replay.js";
...
export function string(value) {
    if (typeof value !== 'string')
        throw new LlmError('DeepSeek Messages expected a string field', 'MALFORMED_RESPONSE');
    return value;
}
...
function indexOf(event) {
    if (!Number.isSafeInteger(event.index) || event.index < 0)
        return malformed('invalid block index');
    return event.index;
}
function updateUsage(usage, raw) {
    const fields = object(raw);
    const keys = { input_tokens: 'inputTokens', output_tokens: 'outputTokens', cache_read_input_tokens: 'cacheReadTokens', cache_creation_input_tokens: 'cacheWriteTokens' };
    for (const [wire, local] of Object.entries(keys)) {
        const value = fields[wire];
        if (value === undefined)
            continue;
        if (!Number.isSafeInteger(value) || value < 0)
            return malformed(`invalid ${wire}`);
        usage[local] = value;
    }
}
    ...
    switch (native.type) {
        ...
        case 'tool_use':
            ...
            if (!content.id || !content.name)
                return malformed('empty tool identity');
 ...
 */
export async function* translate(events, model) {
    ...
    for await (const event of events) {
        if (event.type === 'message_start') {
            if (started)
                return malformed('duplicate message_start');
        ...
        if (!started)
            return malformed('event precedes message_start');
        if (event.type === 'content_block_start') {
            const wireIndex = indexOf(event);
            if (blocks.has(wireIndex) || reason !== undefined)
                return malformed('block starts after settlement or repeats an index');
            ...
            if (block.content.type === 'text' || block.content.type === 'reasoning') {
                if (block.content.text)
                    yield { type: block.content.type === 'text' ? 'text-delta' : 'reasoning-delta', index: block.index, text: block.content.text };
            }
            else {
            ...
            if (block === undefined || block.closed)
                return malformed('delta/stop without an open block');
            ...
            if (delta.stop_reason != null)
                reason = stopReason(delta.stop_reason);
            if (event.usage !== undefined)
                updateUsage(usage, event.usage);
        }
        else {
            if (reason === undefined || [...blocks.values()].some(block => !block.closed))
                return malformed('message_stop without settled blocks and stop reason');
            if (blocks.size === 0 && reason.kind === 'stop')
                throw new LlmError('DeepSeek Messages returned no content', 'EMPTY_RESPONSE');
