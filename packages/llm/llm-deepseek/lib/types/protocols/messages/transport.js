 ...
 */
export function providerErrorDetail(raw) {
    const error = typeof raw === 'object' && raw !== null && 'error' in raw ? raw.error : undefined;
    if (typeof error !== 'object' || error === null)
        return '';
    const fields = error;
    return [fields.code, fields.type, fields.message].filter((value) => typeof value === 'string').join(' ');
}
 ...
 */
export function providerError(raw, status, headers) {
    ...
    if (status === 401 || status === 403 || ['authentication_error', 'permission_error'].includes(type))
        code = 'AUTH';
    else if (isQuotaExceededError(detail) || status === 402)
        code = 'QUOTA';
    else if (status === 429 || type === 'rate_limit_error')
        code = 'RATE_LIMIT';
    else if (isContextWindowExceededError(detail))
        code = 'CONTEXT_WINDOW_EXCEEDED';
    else if (status === 400 || status === 413 || type === 'invalid_request_error')
        code = 'INVALID_REQUEST';
    else if ((status !== undefined && status >= 500) || ['api_error', 'overloaded_error'].includes(type))
        code = 'SERVER';
    else
        code = status === undefined ? 'SERVER' : `HTTP_${status}`;
