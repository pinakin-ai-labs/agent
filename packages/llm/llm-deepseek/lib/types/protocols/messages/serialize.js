...
import { object, readReplay } from "./replay.js";
 ...
 */
export function serialize(options, connection, history, images, access, onReplayDegrade, fileIds) {
    ...
    const input = (blocks) => blocks.flatMap((block) => {
        ...
        if (block.type !== 'image')
            return unsupported(`user/tool-result content ${block.type}`);
        const version = images.get(block.attachment.attachmentId);
        if (version === undefined)
            throw new LlmError('DeepSeek Messages request image is missing', 'INVALID_REQUEST');
        const fileId = fileIds?.get(block.attachment.attachmentId);
        if (fileIds !== undefined && fileId === undefined)
            throw new LlmError('DeepSeek Messages request file id is missing', 'INVALID_REQUEST');
        return [
    ...
    const flushSystemUpdates = () => {
        ...
        if (messages.at(-1)?.role !== 'user')
            return unsupported('system update without a preceding user or tool-result turn');
        messages.push(...systemUpdates.splice(0));
    };
    for (const message of history) {
        if (message.role === 'system') {
            const texts = message.content.filter(block => block.type === 'text');
            if (texts.length !== message.content.length)
                return unsupported('non-text system message');
            const text = texts.map(block => block.text).join('');
            if (inHistory && messages.length > 0) {
                if (text.length === 0)
                    return unsupported('empty in-history system update');
    ...
    for (const message of messages) {
        if (message.role === 'assistant') {
            ...
            if (pending.size > 0)
                throw new LlmError('DeepSeek Messages tool calls need immediate results', 'INVALID_REQUEST');
            message.content = [...results, ...message.content.filter(block => block.type !== 'tool_result')];
        }
    }
    if (pending.size > 0)
        throw new LlmError('DeepSeek Messages history ends with unresolved tools', 'INVALID_REQUEST');
    ...
    return {
        ...
        ...effort === 'off' ? {} : { output_config: { effort: effort } },
