...
import { resolveRequestImageTarget } from "../../common/request-pricing.js";
export { deepSeekImageRequestPricing as imagePricing } from "../../common/request-pricing.js";
...
function* imageRefs(blocks) {
    for (const block of blocks) {
        if (block.type === 'image')
            yield block.attachment;
        else if (block.type === 'tool-result')
            yield* imageRefs(block.content);
    }
}
 ...
 */
export async function prepareImages(history, connection, modelId, attachments, access, signal) {
 ...
 */
export function inlineImages(messages, versions, connection) {
...
function assertImagesFit(messages, versions, connection, representation) {
    ...
    if (offloadImages > 0) {
        throw new LlmError(`DeepSeek Messages ${representation} request images exceed the route budget; ${offloadImages} more oldest occurrence(s) must be offloaded.`, IMAGE_OFFLOAD_REQUIRED_CODE, { offloadImages });
    }
}
 ...
 */
export async function prepareFileIds(messages, versions, files) {
    const ids = new Map();
