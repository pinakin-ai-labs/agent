...
function writableStore(ctx) {
    ...
    if (credentials === undefined) {
        throw new LlmError('llm-pi-ai: this composition mounts no credentials service, so there is nowhere to store the'
    ...
    return {
        ...
        async list() {
            ...
            for (const entry of stored) {
                ...
                if (credentialKeyScope(entry.key) !== RECORD_SCOPE)
                    continue;
                mine.push({
        ...
        async modify(providerId, mutate) {
            if (!isCredentialKeySegment(providerId)) {
                throw new LlmError(`llm-pi-ai: provider id "${providerId}" cannot address a stored credential record (a record id is a`
