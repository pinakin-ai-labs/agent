...
import { presentDefineCall, presentInspectListCall, presentInspectQueryCall, presentInspectSelfCall, presentRunCall, presentStopCall, presentUndefineCall, } from "./present.js";
import { CORDIS_SYSTEM_PROMPT } from "./prompt.js";
...
export function apply(ctx) {
    ...
    ctx.tools.register(defineTool({
        ...
        execute(args, exec) {
            const plugin = args.plugin.kind === 'new'
                ...
                : { kind: 'existing', pluginId: CordisDynamicPluginId(args.plugin.pluginId) };
    ...
    ctx.tools.register(defineTool({
        name: 'cordis_stop',
        description: 'Stop the current Run of a dynamic Plugin and cancel unfinished approval or activation requests. Retain the '
...
function selfState(reference) {
    ...
    if (status === 'client-pending' || status === 'starting-host')
    ...
    if (status === 'running')
        return 'running';
    if (reference.activeRun !== undefined)
        return 'running';
