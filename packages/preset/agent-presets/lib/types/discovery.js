...
async function rowResolves(row, presetBase, harnessBase, resolves) {
    if (row.kind === 'builtin')
        return true;
    if (row.kind === 'package')
        return isBuiltin(row.specifier) || resolves(row.specifier, harnessBase);
...
async function unresolvableRows(rows, presetBase, harnessBase, resolves, at = '') {
    ...
    for (const [index, entry] of rows.entries()) {
        ...
        if (await rowResolves(classifyRowSpecifier(row.name), presetBase, harnessBase, resolves))
            continue;
 ...
 */
export async function scanRoot(root, harnessBase, resolves = packageInstalled) {
 ...
 */
export async function discoverPresets(roots, harnessBase, resolves = packageInstalled) {
    const byId = new Map();
