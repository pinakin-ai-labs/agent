 ...
 */
import { isSupportedProxyUrl, POLICY_ENV_NAMES, PROXY_ENV_NAMES, proxyForUrl, resolveProxyPolicy, } from "./policy.js";
...
async function createPolicyDispatcher(policy) {
    ...
    return new Agent({
        factory(origin, options) {
            ...
            if (proxy !== undefined)
                return new ProxyAgent({ ...passed, uri: proxy });
...
export function proxyEnvironmentForChild() {
    ...
    if (policy === undefined || policy.source === 'none' || inherited === undefined)
        return {};
    ...
    for (const [field, names] of Object.entries(POLICY_ENV_NAMES)) {
        ...
        for (const name of names)
            overlay[name] = named ? inherited[name] : resolved;
    }
