 ...
 */
export function bootstrapArgs(fs, config, maxMessageBytes) {
    if (config.bootstrapPath !== undefined)
        return [config.bootstrapPath, String(maxMessageBytes)];
    if ('pkg' in process)
        return [String(maxMessageBytes)];
    const mapped = (path) => {
        const result = fs.processPathFromHostPath(path);
        if (result === undefined)
            throw new Error(`PTC runtime bootstrap is unavailable in the subprocess execution world: ${path}`);
        return result;
    };
