...
import { bootstrapArgs } from "./launch.js";
import { OutputLedger } from "./output-ledger.js";
import { drainOutput } from "./output-stream.js";
import { STARTUP_ENVIRONMENT_NAMES } from "./environment.js";
import { decodePtcJsonWire, encodePtcJsonWire } from "./json-wire.js";
...
export class NodePtcRuntime extends PtcRuntime {
    ...
    constructor(ctx, config) {
        super(ctx);
        this.config = { ...config, nodeExecutable: config.nodeExecutable ?? process.execPath };
        for (const [key, value] of Object.entries(this.config)) {
            if (typeof value === 'number' && (!Number.isFinite(value) || value <= 0))
                throw new Error(`ptc-runtime-node: ${key} must be positive and finite`);
        }
        for (const key of ['timeoutMs', 'maxTimeoutMs', 'graceMs']) {
            if (this.config[key] > MAX_TIMER_DELAY_MS)
                throw new Error(`ptc-runtime-node: ${key} exceeds the supported timer range`);
        }
        if (!Number.isSafeInteger(this.config.maxOutputBytes) || this.config.maxOutputBytes < 4)
            throw new Error('ptc-runtime-node: maxOutputBytes must be an integer of at least 4');
        if (!Number.isSafeInteger(this.config.maxMessageBytes) || this.config.maxMessageBytes > 0xffff_ffff)
            throw new Error('ptc-runtime-node: maxMessageBytes must fit an unsigned 32-bit frame length');
        if (!Number.isSafeInteger(this.config.maxPendingCalls))
            throw new Error('ptc-runtime-node: maxPendingCalls must be an integer');
        if (!Number.isSafeInteger(this.config.maxOldGenerationSizeMb))
            throw new Error('ptc-runtime-node: maxOldGenerationSizeMb must be an integer');
        if (this.config.nodeExecutable.length === 0)
            throw new Error('ptc-runtime-node: nodeExecutable must be non-empty');
        if (this.config.bootstrapPath !== undefined && !isAbsolute(this.config.bootstrapPath))
            throw new Error('ptc-runtime-node: bootstrapPath must be absolute');
        ctx.effect(() => async () => {
            this.disposed = true;
    ...
    resolve(request) {
        if (this.disposed)
            throw new Error('ptc-runtime-node: resolve after disposal');
    ...
    async run(spec) {
        if (this.disposed)
            throw new Error('ptc-runtime-node: run after disposal');
        if (spec.sandboxPolicy === undefined)
            throw new Error('ptc-runtime-node: run requires a resolved sandbox policy');
        if (!isAbsolute(spec.cwd) || (spec.timeoutMs !== null && (!Number.isFinite(spec.timeoutMs) || spec.timeoutMs <= 0 || spec.timeoutMs > this.config.maxTimeoutMs)))
            throw new Error('ptc-runtime-node: run requires resolved cwd and timeout');
    ...
    async execute(spec, policy, bindings, controller) {
        ...
        const logs = [];
        const sandbox = { mode: policy.mode, denied: false };
        ...
        try {
            ...
            handle.stdout?.on('error', (error) => { finish({ kind: 'worker-exit', message: messageOf(error) }); });
            ...
            handle.stderr?.on('error', (error) => { finish({ kind: 'worker-exit', message: messageOf(error) }); });
            let ready = false;
            let nextId = 1;
            let pending = 0;
            let pendingBytes = 0;
            const protocolFailure = (message) => { finish({ kind: 'protocol', message }); };
            ...
            const transport = new JsonChannel(launched.control, this.config.maxMessageBytes, (raw, bytes) => {
                ...
                switch (raw.type) {
                    ...
                    case 'done': {
                        if (raw.error !== undefined) {
                            ...
                            const failure = { kind: raw.error.kind, message: raw.error.message };
                            if (confined !== undefined) {
                                sandbox.denied = confined.denialSignatures.some(signature => failure.message.toLowerCase().includes(signature.toLowerCase()));
                            }
                        ...
                        if (raw.value !== undefined && value === undefined)
                            finish({ kind: 'invalid-output', message: 'program completion must be lossless JSON' });
                        else
                    ...
                    case 'call': {
                        ...
                        void (async () => {
                            ...
                            try {
                                ...
                                if (value === undefined)
                                    throw new Error('binding resolution must be lossless JSON');
                ...
                if (kind === 'protocol')
                    ...
                    finish({ kind: 'worker-exit', message: messageOf(error) });
                else
                    void launched.done.then(processFinished, (failure) => { finish({ kind: 'worker-exit', message: messageOf(failure) }); });
            });
            channel = transport;
            ...
            }, (error) => { finish({ kind: confined !== undefined && isRunnerSpawnFailure(error, confined.argv[0], spec.cwd) ? 'sandbox-unavailable' : 'worker-exit', message: messageOf(error) }); });
        }
        catch (error) {
