 ...
 */
export function snapshotPtcJsonValue(value) {
    ...
    for (let task = takeLast(tasks); task !== undefined; task = takeLast(tasks)) {
        ...
        if (intrinsicArrayIsArray(candidate)) {
            ...
            if (intrinsicReflectOwnKeys(candidate).length !== length + 1)
                return undefined;
            const target = [];
 ...
 */
export function encodePtcJsonWire(value) {
    const wire = [];
    const pending = [value];
 ...
 */
export function decodePtcJsonWire(input) {
    try {
        ...
        for (let tokenIndex = 0; tokenIndex < wire.length; tokenIndex++) {
            ...
            if (token === null || typeof token === 'boolean' || typeof token === 'string') {
                ...
                if (marker.kind === 'array') {
                    ...
                    value = target;
                    if (marker.length > 0)
                        frame = { kind: 'array', target, length: marker.length, index: 0 };
                }
                else {
                    ...
                    if (marker.keys.length > 0)
                        frame = { kind: 'object', target, keys: marker.keys, index: 0 };
                }
            }
            if (!attach(value))
                return undefined;
...
/* jscpd:ignore-end */
//# sourceMappingURL=json-wire.js.map