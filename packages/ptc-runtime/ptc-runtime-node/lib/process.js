...
*/
function snapshotPtcJsonValue(value) {
	...
	push(text) {
		...
		if (stringBytes === void 0) {
			...
			if (prefix.length > 0) {
				...
				if (prefixBytes === void 0) throw new CapturedError("program output ledger produced an oversized log prefix");
				...
				return new Promise((resolve, reject) => {
					...
					try {
						port.postMessage({
							...
							global,
							name,
							args: encodePtcJsonWire(detached)
						});
					} catch (error) {
						pending.delete(id);
...
*/
async function runProgram(port, data, streams) {
...
*/
async function runNodeMain(stream, maxMessageBytes, processState) {
	if (!Number.isSafeInteger(maxMessageBytes) || maxMessageBytes <= 0 || maxMessageBytes > 4294967295) throw new Error("invalid control message limit");
	for (const key of Object.keys(processState.env)) if (!STARTUP_ENVIRONMENT_NAMES.has(key.toUpperCase())) Reflect.deleteProperty(processState.env, key);
	...
	const channel = new JsonChannel(stream, maxMessageBytes, (raw) => {
		if (!started) {
			...
			if (typeof raw !== "object" || raw === null || raw.type !== "boot") {
				boot.reject(/* @__PURE__ */ new Error("expected program boot frame"));
				return;
			}
...
//#endregion
//#region lib/types/process-entry.js
/** Private built Node-runtime child entry; source execution calls runNodeMain directly. */
runNodeMain(openInheritedControlChannel(), Number(process.argv[2]), process);
//#endregion
export {};
