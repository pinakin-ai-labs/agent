...
import { checkDoneValue, encodeJsonPlain, hasUnsafeIntegerToken, logTruncationMarker, validateChildFrame } from "./protocol.js";
 ...
 */
export function hostFrameParseCeiling(heapLimit = getHeapStatistics().heap_size_limit) {
 ...
 */
export function readProcessStart(pid) {
 ...
 */
export function resolvePythonBin(bin) {
    ...
    if (isAbsolute(bin))
        return executableFile(bin);
    if (bin.includes('/'))
        return executableFile(resolve(bin));
...
const MIN_CPYTHON = { major: 3, minor: 10 };
...
function capMessage(message, maxValueBytes) {
    ...
    if (message.length * 3 <= maxValueBytes)
        return message;
    ...
    if (whole && bytes.length <= maxValueBytes)
        return message;
 ...
 */
export function detachResidual(residual) {
...
export class PythonPtcRuntime extends PtcRuntime {
    ...
    resolve(request) {
        if (request.sandboxPolicy !== undefined)
            throw new Error('dsh-ptc-runtime-python: sandbox policy is unsupported');
        if (request.timeoutMs !== undefined)
            throw new Error('dsh-ptc-runtime-python: per-call timeout is unsupported');
        const cwd = request.cwd ?? process.cwd();
        if (!isAbsolute(cwd))
            throw new Error('dsh-ptc-runtime-python: cwd must be absolute');
    ...
    async run(request) {
        if (request.sandboxPolicy !== undefined || request.timeoutMs !== this.config.maxWallMs)
            throw new Error('dsh-ptc-runtime-python: unsupported execution policy or timeout');
        if (this.disposed)
            throw new Error('dsh-ptc-runtime-python: run() after disposal');
    ...
    execute(request, bindings, bootstrapPath) {
        ...
        try {
            ...
            return Promise.resolve({ logs: [], error: { kind: 'worker-exit', message: `python spawn error: ${messageOf(error)}` } });
        }
        return new Promise((resolve) => {
            ...
            const strayOut = { chunks: [], blocks: [], cost: 0, utf8: { expected: 0, width: 0, lowerFirst: 0, upperFirst: 0 } };
            const strayErr = { chunks: [], blocks: [], cost: 0, utf8: { expected: 0, width: 0, lowerFirst: 0, upperFirst: 0 } };
            ...
            function flushStray(stray, retainPartialTail) {
                if (stray.chunks.length === 0 && stray.blocks.length === 0)
                    return;
            ...
            child.stdout.on('data', (chunk) => { captureStray(strayOut, chunk); });
            child.stderr.on('data', (chunk) => { captureStray(strayErr, chunk); });
            ...
            proto.on('data', (chunk) => {
                ...
                if (chunk.includes(0x0a)) {
                    ...
                    for (const b of sealedBlocks)
                        firstFrameLen += b.length;
            ...
            const killGroup = (sig) => {
                try {
                    ...
                    if (leaderStarted !== undefined && nowStarted !== undefined && nowStarted !== leaderStarted)
                        return;
            ...
            const live = {
                kill,
                finished,
                settle: (failure) => { finish({ error: failure }); },
            };
            this.live.add(live);
            /* jscpd:ignore-end */
