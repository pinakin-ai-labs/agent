...
export const NETWORK_TOPICS = FETCH_TOPICS;
/**
...
export function installFetchObserver(publisher, options) {
    ...
    if (typeof original !== 'function')
        throw new Error('inspector: globalThis.fetch is unavailable');
    ...
    const observedFetch = async (input, init) => {
        ...
        if (requestClone !== undefined) {
            track(captureBody(requestClone.body, options.maxRequestBodyBytes, options.maxChunkBytes, controller.signal, (data) => { publisher.publish('fetch/request-body-chunk', { requestId, data }); }).then((outcome) => {
        ...
        try {
            ...
            track(captureBody(responseClone.body, options.maxResponseBodyBytes, options.maxChunkBytes, controller.signal, (data) => { publisher.publish('fetch/response-body-chunk', { requestId, data }); }).then((outcome) => {
