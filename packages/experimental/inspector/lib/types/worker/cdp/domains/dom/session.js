...
import { realmObjectExpression } from "../../../../shared/cordis/object-registry.js";
import { respondToCdpRequest } from "../../protocol.js";
import { cdpNumericId, cdpStringId, } from "../../ids.js";
...
export class CordisDomSession {
    ...
    backend;
    runtime;
    nodeIdByBackend = new Map();
    backendByNodeId = new Map();
    childrenSent = new Set();
    backendByObjectId = new Map();
    objectIdsByGroup = new Map();
    searches = new Map();
    unsubscribe;
    nextNodeId = 1;
    nextSearchId = 1;
     ...
     */
    handle(request) {
        if (!request.method.startsWith('DOM.'))
            return false;
    ...
    releaseObject(objectId) {
        ...
        for (const ids of this.objectIdsByGroup.values())
            ids.delete(id);
    }
    /**
    ...
    releaseObjectGroup(group) {
        if (typeof group !== 'string')
            return;
        for (const objectId of this.objectIdsByGroup.get(group) ?? [])
    ...
    async execute(method, params) {
        if (READ_ONLY_METHODS.has(method))
            throw new Error('Cordis DOM projection is read-only');
        switch (method) {
            case 'DOM.enable':
                this.enabled = true;
                return {};
            ...
            case 'DOM.pushNodesByBackendIdsToFrontend': {
                if (!Array.isArray(params.backendNodeIds))
                    throw new Error('backendNodeIds must be an array');
                return {
            ...
            case 'DOM.requestNode': {
                ...
                if (binding === undefined)
                    throw new Error('RemoteObject is not a current Cordis node');
                const node = this.backend.document().byBackendId.get(binding.backendNodeId);
                if (node === undefined)
                    throw new Error('Cordis node is no longer available');
                this.pushNodePath(node);
    ...
    async resolveNode(node, objectGroup) {
        ...
        if (route === undefined)
            throw new Error('Structural Cordis node has no live Runtime object');
        if (route.connection.state === 'disconnected')
            throw new Error('Cordis realm is disconnected');
        ...
        if (typeof rawObjectId !== 'string')
            throw new Error('Cordis object lookup returned no RemoteObjectId');
    ...
    bindObjectId(objectId, node, group) {
        ...
        if (source === undefined)
            throw new Error('Structural Cordis node cannot bind a Runtime object');
        ...
        if (ids === undefined)
            this.objectIdsByGroup.set(group, ids = new Set());
        ids.add(objectId);
    }
    selectNode(params) {
        if (params.nodeId !== undefined)
            return this.fromNodeId(params.nodeId);
    ...
    fromNodeId(value) {
        ...
        const node = backendId === undefined ? undefined : this.backend.document().byBackendId.get(backendId);
        if (node === undefined)
            throw new Error('Cordis NodeId is not available in this document');
        return node;
    }
    ...
    pushNodePath(node) {
        ...
        for (const ancestor of chain) {
            if (this.childrenSent.has(ancestor.backendNodeId))
                continue;
    ...
    sendMutation(mutation) {
        switch (mutation.type) {
            ...
            case 'child-removed': {
                ...
                this.forgetSubtree(mutation.node);
                if (parentNodeId === undefined || nodeId === undefined)
                    return;
    ...
    pruneDocumentState() {
        ...
        for (const [backendNodeId, nodeId] of this.nodeIdByBackend) {
            if (document.byBackendId.has(backendNodeId))
                continue;
        ...
        for (const backendNodeId of this.childrenSent) {
            if (!document.byBackendId.has(backendNodeId))
                this.childrenSent.delete(backendNodeId);
        }
        for (const [objectId, binding] of this.backendByObjectId) {
            const node = document.byBackendId.get(binding.backendNodeId);
            const source = node?.object?.source;
            if (source?.sourceId === binding.sourceId && source.generation === binding.generation)
                continue;
            this.backendByObjectId.delete(objectId);
    ...
    releaseSourceObjects(source) {
        for (const [objectId, binding] of this.backendByObjectId) {
            if (binding.sourceId !== source.sourceId || binding.generation !== source.generation)
                continue;
            this.backendByObjectId.delete(objectId);
            for (const [group, objectIds] of this.objectIdsByGroup) {
                objectIds.delete(objectId);
...
function depthParam(value, fallback) {
    ...
    if (!Number.isSafeInteger(value) || value < 1)
        throw new Error('depth must be -1 or a positive integer');
    return value;
}
