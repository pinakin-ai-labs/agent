...
export class NetworkDomain {
            ...
            case 'Network.streamResourceContent': {
                ...
                if (!body.complete) {
                    ...
                    if (requests === undefined)
                        this.streamedRequests.set(session, requests = new Set());
    ...
    receive(event) {
        if (event.type === 'request-evicted') {
            ...
            for (const requests of this.pendingStarts.values())
                requests.delete(event.requestKey);
            for (const requests of this.requestTypes.values())
                requests.delete(event.requestKey);
            return;
        }
        for (const session of this.enabled)
            this.send(session, event);
    }
    send(session, event) {
