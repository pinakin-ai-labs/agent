...
import { INSPECTOR_PROTOCOL_VERSION, parseSourceFrame, } from "../../shared/bridge/messages/observation.js";
...
export class InspectorSourceRegistry {
    ...
    disconnect(connection, reason) {
        for (const [sourceId, state] of this.sources) {
            ...
            for (const consumer of this.consumers)
                consumer.close(state.source, reason);
    ...
    send(source, frame) {
        ...
        if (state === undefined || state.source.generation !== source.generation)
            return false;
        if (jsonByteLength(frame) > this.maxFrameBytes) {
    ...
    close() {
        for (const state of this.sources.values()) {
            for (const consumer of this.consumers)
                consumer.close(state.source, 'inspector worker stopped');
    ...
    apply(connection, frame) {
        ...
        if (frame.t === 'source/close') {
            ...
            for (const consumer of this.consumers)
                consumer.close(state.source, 'source closed');
        ...
        if (frame.t === 'source/replace') {
            ...
            for (const consumer of this.consumers)
                consumer.replace(state.source, frame.records.map((record, index) => ({ ...record, sequence: frame.nextSequence + index })));
        ...
        for (const consumer of this.consumers)
            consumer.append(state.source, records);
        this.count(state, frame.records);
        connection.send({
            v: INSPECTOR_PROTOCOL_VERSION,
    ...
    open(connection, source, topics) {
        if (source.kind !== connection.kind)
            throw new Error('inspector protocol: source kind does not match its carrier');
        const accepted = new Set(topics);
        const prior = this.sources.get(source.sourceId);
        if (prior !== undefined) {
            for (const consumer of this.consumers)
                consumer.close(prior.source, 'source generation replaced');
