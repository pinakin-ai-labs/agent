...
import { parseInspectorObjectReference } from "../../../cordis/object-reference.js";
/**
 ...
 */
export function parseClientRuntimeResult(value) {
    ...
    switch (value.op) {
        ...
        case 'get-properties': {
            ...
            if (!Array.isArray(value.properties))
                throw new Error('inspector protocol: properties must be an array');
 ...
 */
export function parseClientRuntimeRemoteObject(value) {
    ...
    const remote = {
        ...
        ...(object === undefined
            ? {}
            : { object: { handle: wireId(object.handle, 'handle') } }),
...
function parseRemoteObjectDescriptor(value) {
    ...
    if (!REMOTE_TYPES.has(record.type)) {
        throw new Error('inspector protocol: invalid Client RemoteObject type');
    }
    if (record.subtype !== undefined && !REMOTE_SUBTYPES.has(record.subtype)) {
...
function parseObjectPreview(value) {
    ...
    if (!REMOTE_TYPES.has(record.type)
        || (record.subtype !== undefined && !REMOTE_SUBTYPES.has(record.subtype))
...
function parsePropertyPreview(value) {
    ...
    if (typeof record.name !== 'string'
        || (record.type !== 'accessor' && !REMOTE_TYPES.has(record.type))
        || (record.subtype !== undefined && !REMOTE_SUBTYPES.has(record.subtype))) {
...
function parseInternalPropertyDescriptor(value) {
    ...
    if (typeof record.name !== 'string')
        throw new Error('inspector protocol: invalid internal property descriptor');
    return {
        name: record.name,
        ...(record.value === undefined ? {} : { value: parseClientRuntimeRemoteObject(record.value) }),
    };
 ...
 */
export function parseClientRuntimeExceptionDetails(value) {
    const record = exactObject(value, [
 ...
 */
export function parseClientRuntimeStackTrace(value) {
    const record = exactObject(value, ['description', 'callFrames', 'parent'], 'stack trace');
    if (!Array.isArray(record.callFrames))
        throw new Error('inspector protocol: stack callFrames must be an array');
    return {
        ...optionalString(record, 'description'),
...
function parseCallFrame(value) {
    ...
    return {
        ...
        ...(record.scriptKey === undefined ? {} : { scriptKey: wireId(record.scriptKey, 'scriptKey') }),
        url: record.url,
        lineNumber: record.lineNumber,
        columnNumber: record.columnNumber,
    };
...
function validateRemoteObject(value) {
    ...
    switch (descriptor.type) {
        ...
        case 'number': {
            ...
            if (hasObject || finite === special)
                throw new Error('inspector protocol: invalid number RemoteObject representation');
            return;
        }
        case 'bigint':
