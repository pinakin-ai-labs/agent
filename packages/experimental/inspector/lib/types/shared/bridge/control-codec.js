...
import { exactKeys, exactObject } from "../validation.js";
/**
 ...
 */
export function parseInspectorWorkerConfig(value) {
    ...
    if (record.host !== '127.0.0.1')
        throw new Error('inspector protocol: Worker host must be 127.0.0.1');
    ...
    if (startPort > 65_535)
        throw new Error('inspector protocol: Worker startPort must not exceed 65535');
    return {
        host: record.host,
        startPort,
 ...
 */
export function parseInspectorHostControl(value) {
    const record = exactObject(value, ['type'], 'Host control message');
    if (record.type !== 'shutdown')
        throw new Error('inspector protocol: unknown Host control message');
 ...
 */
export function parseInspectorWorkerControl(value) {
    ...
    switch (record.type) {
        ...
        case 'failure':
            ...
            if (typeof record.message !== 'string')
                throw new Error('inspector protocol: invalid Worker failure');
 ...
 */
export function parseInspectorClientBootstrap(value) {
    const record = exactObject(value, [
...
function natural(value, label, zero = false) {
