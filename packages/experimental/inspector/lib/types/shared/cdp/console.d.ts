...
export type RuntimeConsoleType = 'log' | 'debug' | 'info' | 'error' | 'warning' | 'dir' | 'dirxml' | 'table' | 'trace' | 'clear' | 'startGroup' | 'startGroupCollapsed' | 'endGroup' | 'assert' | 'profile' | 'profileEnd' | 'count' | 'timeEnd';
...
export type RuntimeConsoleBackendEvent<Handle extends string> = {
    readonly type: 'console-api';
    readonly event: RuntimeConsoleEvent<Handle>;
} | {
    readonly type: 'exception';
    readonly event: RuntimeExceptionEvent<Handle>;
};
