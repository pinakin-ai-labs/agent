...
export type RuntimeRemoteObjectType = 'object' | 'function' | 'undefined' | 'string' | 'number' | 'boolean' | 'symbol' | 'bigint';
/** Runtime object subtype hints understood by Chrome DevTools. */
export type RuntimeRemoteObjectSubtype = 'array' | 'null' | 'node' | 'regexp' | 'date' | 'map' | 'set' | 'weakmap' | 'weakset' | 'iterator' | 'generator' | 'error' | 'proxy' | 'promise' | 'typedarray' | 'arraybuffer' | 'dataview' | 'webassemblymemory' | 'wasmvalue';
...
//# sourceMappingURL=remote-object.d.ts.map