 ...
 */
export async function request(target, method, args, signal) {
    ...
    try {
        ...
        port1.once('message', (raw) => {
            ...
            if (!response.success)
                ...
                result.resolve(response.data.value);
            else
                result.reject(new Error(response.data.error));
        });
 ...
 */
export async function answer(raw, execute) {
