...
function filteredUserEntries(seq, source, content, initialPromptSeq, parentSession) {
	return content.filter((block) => block.type !== "tool-result").map((block) => ({
		kind: "user-message",
...
async function classifyRisk(ctx, agent, exec, signal) {
	...
	const options = deepFreeze({
		...
		messages: [createUserMessage({
			...
			source: {
				kind: "plugin",
				plugin: "dsh-experimental-auto-review"
			}
		})],
		temperature: 0,
		signal
...
function apply(ctx) {
	...
	ctx.effect(function* () {
		yield ctx.on("tools/pre-execute", async (exec, next) => {
			...
			try {
				const decision = await classifyRisk(ctx, agent, exec, AbortSignal.any([exec.signal, lifecycle.signal])).catch(() => void 0);
				if (isAborted(lifecycle.signal)) return { kind: "cancel" };
				if (decision === void 0) return denied(exec);
