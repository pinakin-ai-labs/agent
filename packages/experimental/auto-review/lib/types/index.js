...
function nativeAction(exec, headerTools, logged) {
    ...
    const schemas = candidates.filter((schema) => isRecord(schema) && schema['name'] === exec.name);
...
function snapshotAutoReview(agent, exec) {
    ...
    for (const event of nativeCalls) {
        ...
        if (bucket === undefined)
            nativeByScopedId.set(key, [event]);
        else
            bucket.push(event);
    }
    ...
    for (const start of starts) {
        ...
        if (bucket === undefined)
            startsByParent.set(parentKey, [start]);
        else
            bucket.push(start);
    }
    ...
    for (const seq of nodes) {
        ...
        if (event.type === 'user/message') {
            if (event.data.source.kind === 'tool')
                continue;
            if (isProjectInstruction(event.data.source)) {
                ...
                history.push(...filteredUserEntries(event.seq, event.data.source, event.data.content, initialPromptSeq, session.header.parentSession));
            }
            continue;
        }
...
async function readDecision(stream) {
    ...
    for await (const chunk of stream) {
        if (finished)
            throw new Error('auto-review: reviewer emitted data after its terminal finish');
        ...
        yield async () => {
            ...
            try {
                for (const session of ctx.sessions.list()) {
                    if (permissionPresets.current(session) !== AUTO_PRESET)
                        continue;
