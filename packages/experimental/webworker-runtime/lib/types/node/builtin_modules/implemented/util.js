    ...
    return (...args) => new Promise((resolve, reject) => {
        fn(...args, (error, value) => {
            if (error !== null && error !== undefined)
                reject(error instanceof Error ? error : new Error(inspect(error)));
            else
                resolve(value);
        });
    });
...
export function callbackify(fn) {
    return (...args) => {
        ...
        fn(...rest).then((value) => { callback(null, value); }, (error) => { callback(error); });
    };
}
/**
...
export function inspect(value) {
    ...
    try {
        ...
        const rendered = JSON.stringify(value, (_key, item) => typeof item === 'bigint' ? item.toString() : item);
