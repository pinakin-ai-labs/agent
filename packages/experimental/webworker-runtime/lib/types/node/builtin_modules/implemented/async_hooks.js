...
export class AsyncLocalStorage {
    ...
    run(store, callback, ...args) {
        ...
        const removeBoundary = () => {
            ...
            if (at !== -1)
                this.ambients.splice(at, 1);
            if (this.resumed === undefined)
                this.resumed = restoreResumed;
            remove();
        };
    ...
    getStore() {
        if (this.overlay !== undefined)
            return this.overlay.store;
        if (this.resumed !== undefined)
            return this.resumed.store;
        const ambient = this.ambients.at(-1);
        if (ambient !== undefined)
            return ambient.store;
    ...
    static restoreAll(snapshot) {
        ...
        return () => {
            for (const { instance, slot, before } of installed) {
                if (instance.resumed === slot)
                    instance.resumed = before;
            }
        };
    }
    /**
    ...
    static runWithContext(snapshot, callback) {
        ...
        try {
            ...
            for (const { instance, before } of previous)
                instance.overlay = before;
        }
    }
    /**
 ...
 */
export function captureAsyncContext() {
 ...
 * @returns the callback's return value.
 */
export function runWithAsyncContext(snapshot, callback) {
 ...
 */
export function bindAsyncContext(callback) {
 ...
 * @returns the callback's return value.
 */
export function runAtAsyncContextRoot(callback) {
    const root = AsyncLocalStorage.liveInstances().map(instance => ({ instance, store: undefined }));
 ...
 */
export function __restoreAll(snapshot) {
...
export const alsCausality = {
    ...
    restore: (snapshot) => { __restoreAll(snapshot); },
};
/**
...
export const AsyncResource = notImplementedFail('node:async_hooks', 'AsyncResource');
...
//# sourceMappingURL=async_hooks.js.map