...
function requireGzipMember(source) {
    ...
    let judged = false;
    const refuse = (read) => new Error(`webworker image: ${source} is not the gzip-compressed tar this deployment serves as its image `
    ...
    return new TransformStream({
        transform: (chunk, controller) => {
            ...
            head = merged;
            if (head.byteLength < GZIP_MAGIC.length)
                return;
            if (GZIP_MAGIC.some((byte, at) => head[at] !== byte))
                throw refuse(head);
            judged = true;
 ...
 */
export async function inflateImageStream(body, source) {
    const inflated = body
 ...
 */
export async function inflateImage(bytes, source) {
    const body = new Response(bytes).body;
    if (body === null)
        throw new Error(`webworker image: ${source} produced no readable body`);
    return await inflateImageStream(body, source);
}
//# sourceMappingURL=image-gzip.js.map