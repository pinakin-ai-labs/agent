...
import { parsePreviewFixtureManifest, } from "../fixture-manifest.js";
 ...
 */
export async function choosePreviewSource(manifestUrl) {
    const requested = new URL(location.href).searchParams.get(PREVIEW_FIXTURE_QUERY);
    if (requested === EMPTY_SOURCE)
        return [];
    ...
    if (root === null)
        throw new Error('preview source chooser: missing #root');
    ...
    if (form === null)
        throw new Error('preview source chooser: form was not rendered');
    const sourceId = await new Promise((resolve, reject) => {
        form.addEventListener('submit', (event) => {
            ...
            if (typeof value === 'string')
                ...
                reject(new Error('preview source chooser: no source selected'));
        }, { once: true });
    });
    const choice = choices.find(candidate => candidate.id === sourceId && candidate.disabled !== true);
    if (choice === undefined)
        throw new Error(`preview source chooser: unavailable source "${sourceId}"`);
    chooser.remove();
    style.remove();
    return choice.overlays;
}
//# sourceMappingURL=source-chooser.js.map