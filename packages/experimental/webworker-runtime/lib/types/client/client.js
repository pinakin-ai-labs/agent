...
export class WorkerTunnel {
    ...
    constructor(worker) {
        ...
        worker.addEventListener('error', (event) => {
            ...
            for (const id of this.inFlight.keys())
                this.warnRefusal(id, `worker failed: ${event.message}`);
            this.inFlight.clear();
            for (const pending of this.unary.values())
                pending.reject(reason);
            this.unary.clear();
            for (const controller of this.bodyStreams.values())
    ...
    fetch = async (input, init) => {
        ...
        try {
            ...
            if (this.bodyStreams.has(id))
                this.observeStreamAbort(id, signal);
            return settled;
        }
        finally {
            raced.release();
    ...
    async *open(endpoint, payload, signal) {
        ...
        try {
            ...
            while (true) {
                ...
                terminal = true;
                if (response.t === 'stream-error')
                    throw new TunnelLogicalStreamError(response.failure);
                return;
            }
        }
        finally {
    ...
    rejectOnAbort(id, signal) {
        ...
        const rejected = new Promise((_resolve, reject) => {
            const fail = () => { reject(this.abortRequest(id)); };
            if (signal.aborted) {
                fail();
                return;
            }
