...
export function normalize(path) {
    ...
    if (absolute)
        return SEP + body + (trailing && body !== '' ? SEP : '');
    if (body === '')
        return trailing ? './' : '.';
...
export function basename(path, suffix) {
    ...
    if (suffix !== undefined && suffix !== name && name.endsWith(suffix))
...
export function relative(from, to) {
    ...
    while (shared < source.length && shared < target.length && source[shared] === target[shared])
        shared += 1;
    const up = new Array(source.length - shared).fill('..');
...
export function fileUrlToPath(url) {
    ...
    if (!text.startsWith('file://'))
        throw new Error(`webworker vfs: not a file URL: ${text}`);
