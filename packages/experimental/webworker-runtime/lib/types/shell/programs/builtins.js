 ...
 */
import { readVariable } from "../expand.js";
import { resolveIn } from "../fs-access.js";
...
const cd = async (argv, io, state, fs) => {
    ...
    if ('PWD' in state.environment)
        state.environment['PWD'] = path;
    return 0;
};
...
const exportProgram = (argv, io, state) => {
    ...
    if (options.operands.length === 0) {
        for (const [name, value] of Object.entries(state.environment).sort())
            io.out(`declare -x ${name}="${value}"\n`);
        return 0;
    }
...
const test = async (argv, io, state, fs) => {
    ...
    if (words.length === 2) {
        ...
        switch (operator) {
            ...
            case '-r':
            case '-w': return status(await statOf(operand) !== undefined);
...
const sleep = async (argv, io, state) => {
    ...
    const killed = await new Promise((settle) => {
...
const printenv = (argv, io, state) => {
    const name = argv[1];
    if (name === undefined) {
        for (const [key, value] of Object.entries(state.environment).sort())
            io.out(`${key}=${value}\n`);
        return 0;
    }
