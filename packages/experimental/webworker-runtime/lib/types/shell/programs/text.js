 ...
 */
import { describeFailure, resolveIn } from "../fs-access.js";
import { numberOption, parseOptions, toLines } from "./options.js";
/**
...
async function readInputs(program, operands, io, state, fs) {
    if (operands.length === 0)
        return { sources: [{ name: '-', text: io.stdin }], status: 0 };
    const sources = [];
    let status = 0;
...
async function walkFiles(path, display, into, fs) {
    for (const entry of await fs.list(path)) {
        ...
        if (entry.directory)
            await walkFiles(child, shown, into, fs);
        else
            into.push({ path: child, display: shown });
    }
}
        ...
        for (const target of targets) {
            ...
            if (stats?.directory === true) {
                ...
                for (const file of files)
                    sources.push({ name: file.display, text: await fs.readText(file.path) });
                continue;
            }
            try {
...
const uniq = async (argv, io, state, fs) => {
    ...
    for (const line of lines) {
        ...
        if (previous !== undefined && previous.text === line)
            previous.count += 1;
        else
...
const sed = async (argv, io, state, fs) => {
    ...
    for (const source of sources) {
        for (const line of toLines(source.text))
            io.out(`${line.replace(matcher, replacement.replace(/\\(\d)/g, '$$$1'))}\n`);
    }
    return status;
};
