...
import { basename, dirname, resolve } from "../../module-system/posix-path.js";
...
const find = async (argv, io, state, fs) => {
    ...
    const visit = async (path, display, depth) => {
        ...
        if (selected)
            io.out(`${display}\n`);
        if (!stats.directory || depth >= maxDepth)
            return;
    ...
    for (const root of roots.length > 0 ? roots : ['.'])
        await visit(resolveIn(state.cwd, root), root, 0);
    return status;
};
...
async function copyTree(from, to, fs) {
    ...
    for (const entry of await fs.list(from))
        await copyTree(resolve(from, entry.name), resolve(to, entry.name), fs);
}
...
const dirnameProgram = (argv, io) => {
    for (const operand of argv.slice(1))
        io.out(`${dirname(operand)}\n`);
