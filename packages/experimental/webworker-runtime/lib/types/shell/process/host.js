...
import { hostFileSystem } from "../fs-access.js";
import { runShellProcess } from "./child.js";
...
function startWorkerProcess(options) {
    ...
    worker.addEventListener('message', (event) => {
        ...
        void serveFilesystemCall(fs, frame.op, frame.args).then((value) => { worker.postMessage({ t: 'fs-reply', id: frame.id, value }); }, (error) => {
            const failure = {
                code: error.code,
    ...
    return {
        interrupt: () => { if (!settled)
            worker.postMessage({ t: 'shell-signal' }); },
...
function startInlineProcess(options) {
    ...
    void run.then((outcome) => { options.onExit(outcome.exitCode); }, (error) => {
        options.onOutput('stderr', `bash: ${String(error)}\n`);
        options.onExit(1);
    });
    const stop = () => { stopping.abort(new Error('killed by signal')); };
