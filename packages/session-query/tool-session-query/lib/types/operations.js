...
async function executeSessionSearch(ctx, args, exec, maxResults) {
    ...
    if (cwd === undefined) {
        throw new HarnessError('cross-session search is unavailable because the caller session has no workspace', 'SESSION_QUERY_TOOL_UNAUTHORIZED');
    }
    ...
    if (requestedParentIds !== undefined || args.include_root_sessions === true) {
        ...
        const parentValues = requestedParentIds
...
async function executeEventSearch(ctx, args, exec, maxResults) {
    ...
    if (sessionId === caller.id) {
        ...
        if (stepStartSeq === undefined) {
            throw new HarnessError('current-session search requires an active step boundary', 'SESSION_QUERY_TOOL_NO_CURRENT_STEP');
        }
...
async function executeEventTrace(ctx, args, exec) {
    ...
    const sessionId = workspaceAccess.targetId(args, caller);
    await workspaceAccess.authorizeTarget(ctx, caller, sessionId, exec.signal);
    const trace = await serviceBoundary.call(ctx, exec.signal, 'event trace', () => ctx.sessionQuery.traceEvent({ sessionId, seq }, exec.signal));
...
async function executeEventRead(ctx, args, exec) {
    ...
    if (args.before !== undefined)
        toolInput.assertNonNegativeSafeInteger('before', args.before);
    if (args.after !== undefined)
        toolInput.assertNonNegativeSafeInteger('after', args.after);
    const caller = workspaceAccess.callerOf(exec, ctx);
    const sessionId = workspaceAccess.targetId(args, caller);
    await workspaceAccess.authorizeTarget(ctx, caller, sessionId, exec.signal);
...
async function collectPages(maxResults, signal, request, accept) {
    ...
    while (true) {
        ...
        if (seen.has(page.nextCursor)) {
            throw new SessionQueryError('session-search provider repeated a continuation cursor', 'SESSION_QUERY_INVALID_CURSOR');
        }
