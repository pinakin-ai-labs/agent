...
import { LlmError, createAssistantMessage, errorChain, markAgentLoopRequest, } from '@deepseek-ai/dsh-llm';
    ...
    runMaintenance(job) {
        ...
        return (async () => {
            try {
                ...
                const cause = maintenance.abort.signal.reason;
                if (cause?.kind !== 'disposed' && maintenance.wakeRequested && this.inbox.hasPending)
                    this.wakeDriver();
                done.resolve();
            }
        })();
