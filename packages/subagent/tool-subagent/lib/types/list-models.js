...
async function listSubagentModels(ctx, policy, request, signal) {
    ...
    const efforts = model.reasoning?.efforts.map(effort => (`${effort.id}${model.reasoning?.defaultEffort === effort.id ? ' (default)' : ''} — ${effort.name}`
        + (effort.description === undefined ? '' : `: ${effort.description}`))).join('\n') || '(no advertised reasoning efforts)';
 ...
 */
export function registerListSubagentModels(ctx, policy) {
