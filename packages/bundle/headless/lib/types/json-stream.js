...
function boundKey(key, maxBytes, state) {
    if (Buffer.byteLength(key, 'utf8') <= maxBytes)
        return key;
 ...
 */
export function boundJsonLine(event, maxStringBytes = MAX_STRING_BYTES, maxEventBytes = MAX_EVENT_BYTES) {
    ...
    if (Buffer.byteLength(short, 'utf8') <= limit)
        return short;
...
function addUsage(state, next) {
    if (next === undefined)
        return { usage: state.usage, complete: false };
    if (state.usage === undefined)
        return { usage: next, complete: state.complete };
 ...
 */
export function projectJsonRun(ctx, agent, sink, options = {}) {
    const maxStringBytes = options.maxStringBytes ?? MAX_STRING_BYTES;
    let disposed = false;
    let stepUsage = { usage: undefined, complete: true };
    const write = (event) => {
            ...
            case 'assistant/message':
                ...
                for (const block of event.data.message.content) {
                    if (block.type === 'reasoning')
                        write({ type: 'thinking', text: block.text });
