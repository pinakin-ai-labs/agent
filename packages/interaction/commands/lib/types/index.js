...
export { CommandDefinitionId, CommandId } from "./brand.js";
...
let CommandRuntime = (() => {
    ...
    return class CommandRuntime extends _classSuper {
        ...
        registerFileReceiptResolver(resolver) {
            ...
            return () => {
                if (this.fileReceipts.resolver === resolver)
                    this.fileReceipts.resolver = undefined;
            };
        }
        /**
...
async function admitCommandAttachments(store, attachments, resolveFileReceipt) {
    ...
    for (const attachment of attachments) {
        if (attachment.type !== 'file' || files.has(attachment.receiptId))
            continue;
    ...
    for (const attachment of attachments) {
        ...
        blocks.push(Object.freeze({
            type: 'file', attachment: files.get(attachment.receiptId),
        }));
    }
