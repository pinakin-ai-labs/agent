...
import { allocateScheduleId, createAfterScheduleRecord, createAtScheduleRecord, createEveryScheduleRecord, foldScheduleEvents, MIN_EVERY_INTERVAL_SECONDS, ScheduleId, ScheduleInputError, ScheduleLogError, scheduleView, } from "./domain.js";
...
const CREATE_DESCRIPTION = 'Create one reminder in the current session. Supply a non-empty prompt and exactly one selector: '
...
const LIST_DESCRIPTION = 'List every active reminder in the current session in creation order, including its exact id, '
    + 'UTC target, scheduled or overdue state, and session-local delivery mode.';
const DELETE_DESCRIPTION = 'Delete one active reminder in the current session by the exact id returned by schedule_create '
                ...
                return runCancellableScheduleTransaction(agent, exec.signal, async () => {
                    ...
                    try {
                        if (args.at !== undefined) {
                            ...
                            record = createEveryScheduleRecord(id, args.prompt, args.every_seconds, Date.now());
                        }
                    }
                    catch (error) {
                    ...
                    if (cancelledBeforeAppend !== undefined)
                        return cancelledBeforeAppend;
                    try {
