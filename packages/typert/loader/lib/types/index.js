...
export function validateTypertManifest(pkgName, exported) {
    ...
    if (manifest.package !== pkgName) {
        throw new Error(`typert-loader: ${pkgName} TYPERT manifest names package ${JSON.stringify(manifest.package)} — the manifest must be owned by the package that exports it`);
    }
    ...
    for (const value of events) {
        ...
        if (event.mode !== undefined && typeof event.mode !== 'string') {
            throw new Error(`typert-loader: ${pkgName} event "${event.name}" mode must be a string`);
        }
    }
...
function requireArray(pkgName, value, subject) {
    if (!Array.isArray(value))
        throw new Error(`typert-loader: ${pkgName} ${subject} must be an array`);
    return value;
}
...
function requireMembers(pkgName, value, subject) {
    for (const item of requireArray(pkgName, value, `${subject}.members`)) {
        ...
        if (typeof member.kind !== 'string' || !MEMBER_KINDS.has(member.kind)) {
            throw new Error(`typert-loader: ${pkgName} ${subject} member "${member.name}" has invalid kind`);
        }
    }
}
...
function requireInvocation(pkgName, value) {
    ...
    if (invocation.scope !== undefined) {
        ...
        if (lookupCount !== 1 || parameter?.source !== 'lookup' || parameter.lookup !== scope.context) {
            throw new Error(`typert-loader: ${pkgName} invocation "${id}" scope wire "${scope.wire}" must select its only lookup parameter`);
        }
    }
    if (receiver.kind === 'context' && wires.has(receiver.wire)) {
        throw new Error(`typert-loader: ${pkgName} invocation "${id}" repeats Context wire field "${receiver.wire}"`);
    }
...
export async function apply(ctx, config) {
    ...
    const resolveArtifact = (pkgName) => {
        ...
        try {
            ...
            if (configured.has(pkgName)) {
                throw new Error(`typert-loader: configured package "${pkgName}" cannot be resolved from the config tree — add it to the composition package dependencies or remove it from packages`, { cause });
            }
    ...
    const flush = (onError) => {
        ...
        for (const entryName of [...dirty]) {
            ...
            try {
                ...
                if (task !== undefined)
                    tasks.push(task.catch((error) => { onError(toError(error)); }));
            }
            catch (error) {
    ...
    ctx.on('internal/plugin', (fiber) => {
        ...
        queueMicrotask(() => {
            ...
            if (!active)
                return;
            for (const task of flush((err) => { ctx.logger.error(err); }))
                void task;
        });
    });
    ...
    if (failures.length > 0) {
        throw new AggregateError(failures, `typert-loader: ${String(failures.length)} typert contributor(s) failed to register:\n${failures.map(e => `  - ${e.message}`).join('\n')}`);
    }
}
