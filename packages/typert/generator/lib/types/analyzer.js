        ...
        for (const face of ['host', 'client']) {
            ...
            for (const reference of aggregate.parsed.projectReferences ?? []) {
                ...
                if (!isDualFacePackage(manifest)) {
                    ...
                    registrations.push({ ...registration, face: 'host', exportSubpaths: hostExportSubpaths(manifest) }, { ...registration, face: 'client', exportSubpaths: clientExportSubpaths(manifest) });
                }
                else {
                    registrations.push({
                        ...registration,
...
class FaceAnalyzer {
        ...
        for (const member of context.members) {
            ...
            if (member.questionToken !== undefined
                || (ts.isUnionTypeNode(member.type)
                    && member.type.types.some(node => node.kind === ts.SyntaxKind.UndefinedKeyword)))
                continue;
            ...
            if (memberOwner?.name !== declarationOwner?.name)
                continue;
            const symbolId = this.symbolId(symbol);
    ...
    invocationModel(registration, binding, method, invocation) {
        ...
        for (const [parameterIndex, parameter] of method.parameters.entries()) {
            ...
            if (lookup !== undefined) {
                if (optional)
                    this.fail(parameter, `lookup parameter for ${lookup.key} cannot be optional`);
            ...
            if (wires.has(modeled.wire))
                this.fail(parameter, `duplicate Remote wire field ${modeled.wire}`);
        ...
        if (invocation.kind === 'context') {
            ...
            if (wires.has(wire))
                this.fail(method, `Remote Scope wire field ${wire} conflicts with a method parameter`);
            receiver = {
                kind: 'context',
        ...
        if (invocation.kind === 'direct') {
            ...
            if (parameter !== undefined && context !== undefined) {
                ...
                if (contextBoundary.typeSymbol !== parameter.boundary.typeSymbol) {
                    this.fail(method, `Remote scope ${context.key} wire type ${contextBoundary.typeSymbol} does not match lookup wire type ${parameter.boundary.typeSymbol}`);
                }
    ...
    gatewayFieldBinding(declaration) {
        const candidates = declaration.members.filter((member) => ts.isPropertyDeclaration(member) && memberName(member.name) === 'typertRemote');
    ...
    gatewayBindingArguments(call, site) {
        ...
        if (!isRemoteSegment(namespace))
            this.fail(options ?? call, 'Gateway namespace must contain only RPC endpoint segment characters');
    ...
    lookupDeclarations() {
        ...
        for (const declaration of this.typeMetaMapMembers('TypertLookupMap')) {
            ...
            if (byKey.has(key))
                this.fail(declaration, `duplicate TypertLookupMap key ${key}`);
            if (byHost.has(entry.hostSymbol))
                this.fail(declaration, `Host type ${host.name} has more than one Typert lookup`);
            byKey.set(key, entry);
    ...
    contextDeclarations() {
        ...
        for (const declaration of this.typeMetaMapMembers('TypertContextMap')) {
            ...
            if (result.has(key))
                this.fail(declaration, `duplicate TypertContextMap key ${key}`);
        ...
        for (const sourceFile of this.program.getSourceFiles()) {
            for (const statement of sourceFile.statements) {
                ...
                for (const nested of statement.body.statements) {
                    if (ts.isInterfaceDeclaration(nested) && nested.name.text === name)
                        result.push(...nested.members);
                }
            }
        }
        return result;
    ...
    resolvedRemoteCodecType(authoredType, resolvedType, topLevelAbsence) {
        ...
        const convert = (type) => {
            ...
            try {
                ...
                if ((flags & ts.TypeFlags.ESSymbol) !== 0)
                ...
                if (this.checker.isArrayType(type) || this.checker.isArrayLikeType(type)) {
                    const element = this.checker.getIndexTypeOfType(type, ts.IndexKind.Number);
                    if (element === undefined)
                        this.fail(authoredType, 'Remote codec array has no element type');
    ...
    assertRemoteJsonType(type, site, active, allowUndefined, allowVoid) {
        ...
        if ((flags & ts.TypeFlags.Undefined) !== 0 && allowUndefined)
            return;
        if ((flags & ts.TypeFlags.Void) !== 0 && allowVoid)
            return;
    ...
    isRemotePhantomConstraint(type) {
        if ((type.flags & ts.TypeFlags.Unknown) !== 0)
            return true;
        if ((type.flags & ts.TypeFlags.Any) !== 0 || (type.flags & ts.TypeFlags.Object) === 0)
            return false;
        if (type.getCallSignatures().length > 0 || type.getConstructSignatures().length > 0)
            return false;
    ...
    resolvedCycleReference(type, site, resolvedType, recursiveDeclarations) {
        ...
        if (owner === undefined)
            this.fail(site, `Remote codec recursive type ${resolved.name} is not owned by this face`);
    ...
    isTypeMetaSymbol(node, name) {
        ...
        for (let current = declaration; current !== undefined; current = optionalParent(current)) {
            if (ts.isModuleDeclaration(current)
                ...
                && current.name.text === '@deepseek-ai/dsh-typert-protocol')
                return true;
        }
        return false;
    }
    validateInvocationIdentity(packages) {
        ...
        for (const invocation of packages.flatMap(packageModel => packageModel.invocations)) {
            ...
            if (existingEndpoint !== undefined) {
                throw new TypertAnalysisError(`typert(${this.face}): ${invocation.location.file}:${String(invocation.location.line)}:${String(invocation.location.column)}: Remote endpoint ${endpoint} conflicts with ${existingEndpoint.id}`);
            }
            const existingId = ids.get(invocation.id);
            if (existingId !== undefined) {
                throw new TypertAnalysisError(`typert(${this.face}): ${invocation.location.file}:${String(invocation.location.line)}:${String(invocation.location.column)}: Remote invocation id ${invocation.id} conflicts with ${existingId.id}`);
            }
    ...
    members(members, ownerId) {
        ...
        for (const member of members) {
            if (ts.isPropertyDeclaration(member)
                ...
                && this.isTypeMetaSymbol(member.initializer.expression, 'bindTypertRemote'))
                continue;
            if (ts.isMethodDeclaration(member) && member.body !== undefined
                && members.some(candidate => candidate !== member
                    ...
                    && (!ts.isMethodDeclaration(candidate) || candidate.body === undefined)))
                continue;
        ...
        if (ts.isClassDeclaration(statement)) {
            for (const member of statement.members) {
                ...
                for (const decorator of ts.canHaveDecorators(member) ? ts.getDecorators(member) ?? [] : []) {
                    ...
                    if (name === 'Remote' || name === 'RemoteScope')
                        return true;
                }
            }
        }
