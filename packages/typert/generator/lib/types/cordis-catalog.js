...
function checkTypeLinks(where, names, policy, violations) {
    for (const name of names) {
        if (Object.hasOwn(policy.linkedTypePages, name)
            ...
            continue;
        violations.push(`${where} references unclassified type '${name}'. Add it to linkedTypePages with its documentation page, `
...
export class CordisCatalogProjector {
    ...
    collectServices() {
        ...
        for (const service of this.renderableServices()) {
            ...
            if (parsedDeclaration.deprecated)
                continue;
 ...
 */
export function projectCordisCatalog(scanRoot, policy, targetFace = 'host') {
 ...
 */
export function collectEvents(scanRoot, policy) {
 ...
 */
export function collectServices(scanRoot, policy) {
...
function checkParams(where, apiKind, parameters, tags, isExempt, violations) {
    for (const parameter of parameters) {
        ...
        if (isExempt(parameter))
            continue;
        const description = tags.get(parameter.name);
        if (description === undefined)
            violations.push(`${where} is missing @param ${parameter.name}.`);
...
function checkReturns(where, signature, returns, renderer, violations) {
    ...
    if (type === 'void' || type === 'Promise<void>')
        return;
    if (returns === null)
        violations.push(`${where} is missing @returns (return type: ${type}).`);
    else if (returns.trim() === '')
        violations.push(`${where}: @returns has an empty description.`);
}
...
function renderParameters(parameters) {
    const values = [...parameters].map(([name, description]) => (`{ name: ${quote(name)}, description: ${quote(description)} }`));
    return `[${values.join(', ')}]`;
}
        ...
        if (service.methods.length === 0) {
            ...
            for (const method of service.methods) {
                ...
                if (contract.returns !== null)
                    lines.push(`        returns: ${quote(contract.returns)},`);
                if (contract.throws.length > 0)
                    lines.push(`        throws: ${quoteList(contract.throws)},`);
                lines.push('      },');
            }
            lines.push('    ],');
        }
 ...
 */
export function renderPageRegion(page, services, events, policy) {
    const lines = [
        REGION_BEGIN,
        '',
    ...
    for (const s of services)
        lines.push(...renderService(s, page, policy.linkedTypePages));
 ...
 */
export function renderInheritedPage(policy) {
    const lines = [
        ...BANNER,
...
//# sourceMappingURL=cordis-catalog.js.map