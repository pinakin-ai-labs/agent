...
async function assertOwnerOnly(filename) {
    ...
    if (offending === 0)
        return;
    throw new Error(`credentials-local: ${filename} is readable beyond its owner (mode ${(mode & 0o777).toString(8)});`
...
export function parseCredentialsDocument(text, filename) {
    ...
    if (document.errors.length > 0) {
        throw new Error(`credentials-local: invalid document at ${filename}: ${document.errors.map(describeYamlError).join('; ')}`);
    }
    ...
    if (!('version' in fields)) {
        throw new Error(`credentials-local: ${filename} uses the pre-release flat layout. Add \`version: ${DOCUMENT_VERSION}\``
    ...
    if (fields['version'] !== DOCUMENT_VERSION) {
        throw new Error(`credentials-local: ${filename} declares version ${JSON.stringify(fields['version'])};`
...
export function renderFlatLayoutMigration(text) {
    ...
    if (!isMap(flat) || flat.items.length === 0)
        return undefined;
    ...
    for (const pair of flat.items) {
        if (!isScalar(pair.key) || typeof pair.key.value !== 'string' || pair.key.value === 'version')
            return undefined;
        try {
...
function parseRecord(key, value, filename) {
    ...
    if (kind === undefined)
        throw new Error(`credentials-local: record "${key}" in ${filename} has no kind`);
...
function parseRecordEnv(key, env, filename) {
    ...
    for (const [name, value] of Object.entries(env)) {
        ...
        if (typeof value !== 'string' || value.length === 0) {
            throw new TypeError(`credentials-local: record "${key}" env "${name}" in ${filename} must be a non-empty string`);
        }
        parsed[name] = value;
    }
    return parsed;
...
function assertJsonValue(where, value, seen) {
    ...
    if (typeof value === 'object') {
        if (seen.has(value))
            throw new TypeError(`credentials-local: ${where} is cyclic`);
...
export class LocalCredentialProvider extends CredentialProvider {
    ...
    async modifyRecord(key, mutate) {
        if (this.isClosed())
            throw new Error(`credentials-local is disposed: cannot modify "${key}"`);
        return this.enqueue(async () => {
            ...
            return withFileLock(this.spec.filename, async () => {
                ...
                if (next.kind === 'grant')
                    assertJsonValue(`record "${key}" payload`, next.payload, new Set());
                else
    ...
    async deleteRecord(key) {
        if (this.isClosed())
            throw new Error(`credentials-local is disposed: cannot delete "${key}"`);
    ...
    assertUnshadowed(ref, verb) {
        if (this.inherited(ref) !== undefined) {
            throw new Error(`credentials-local: "${ref}" is supplied read-only by the launching environment, so ${verb} would be`
    ...
    async migrateFlatDocument() {
        return withFileLock(this.spec.filename, async () => {
            ...
            this.ctx.logger.info('credentials-local: migrated %s to the version %d layout; values are unchanged', this.spec.filename, DOCUMENT_VERSION);
            return migrated;
        }, { waitMs: DOCUMENT_LOCK_WAIT_MS });
    }
