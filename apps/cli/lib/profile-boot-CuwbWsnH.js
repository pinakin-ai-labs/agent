...
import { PROFILE_PATCH_FILENAME, PROFILE_TEMPLATES, PluginPackages, boot, composeEntries, createProfileResolutionGeneration, healProfilesModuleFallback, initProfile, installFailLoud, loadOptionalPatches, loadOverlayPatches, loadProfile, resolveProfileDir, watchUserPatches } from "@deepseek-ai/dsh-app-boot";
...
async function composeProfile(name, patchFiles, resolutionMode, fromDefaultProfile) {
	...
	const resolution = resolutionMode === "runtime" ? await createProfileResolutionGeneration(resolutionOptions) : await healProfilesModuleFallback(resolutionOptions);
...
async function runProfile(options) {
	...
	const resolutionMode = process.pkg !== void 0 ? "runtime" : options.resolutionMode ?? "link";
