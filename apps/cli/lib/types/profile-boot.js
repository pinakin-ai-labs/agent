...
import { boot, composeEntries, createProfileResolutionGeneration, healProfilesModuleFallback, initProfile, installFailLoud, loadOptionalPatches, loadOverlayPatches, loadProfile, PluginPackages, PROFILE_PATCH_FILENAME, PROFILE_TEMPLATES, resolveProfileDir, watchUserPatches, } from '@deepseek-ai/dsh-app-boot';
 ...
 */
export function initializeProfileFromDefault(name, fromDefaultProfile, home = resolveDshHome()) {
    ...
    try {
        ...
        if (existsSync(manifestPath)) {
            throw new Error(`${NAME}: profile ${JSON.stringify(name)} already exists at ${manifestPath}; `
                + 'omit --from-default-profile to use it');
        }
 ...
 */
export function prepareProfile(name, userLayer = true, fromDefaultProfile) {
    if (fromDefaultProfile !== undefined)
        initializeProfileFromDefault(name, fromDefaultProfile);
 ...
 */
async function composeProfile(name, patchFiles, resolutionMode, fromDefaultProfile) {
...
function suppressShutdownError(ctx, signal, error) {
    if (signal.aborted)
        return;
    if (ctx.fiber.state !== 2 /* FiberState.ACTIVE */ || ctx.get('loader') === undefined)
        return;
    throw error;
}
/**
...
export async function runProfile(options) {
    ...
    const disposeProxy = await installProxyFromEnvironment(options.environment, (message) => { process.stderr.write(`${NAME}: ${message}\n`); });
    const packaged = process.pkg !== undefined;
    const resolutionMode = packaged ? 'runtime' : options.resolutionMode ?? 'link';
    const composed = await composeProfile(options.profile, options.patchFiles, resolutionMode, options.fromDefaultProfile);
    const app = {};
    ...
    if (composed.profile.patchReload === 'live'
        ...
        && ctx.fiber.state === 2 /* FiberState.ACTIVE */
...
//# sourceMappingURL=profile-boot.js.map