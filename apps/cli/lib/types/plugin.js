...
import { DEFAULT_PROFILE_BUNDLES, initProfile, PROFILE_TEMPLATES, readProfileManifest, resolveBundleDir, resolveProfileDir, writeProfileManifest, } from '@deepseek-ai/dsh-app-boot';
...
function reconcilePlugins(before, profileDir) {
    ...
    for (const packageName of dependencies) {
        ...
        if (isBundle && !plugins.includes(packageName)) {
            ...
            process.stderr.write(`${NAME}: warning: ${packageName} declares no dsh.bundle — installed as a plain dependency, not a profile layer `
...
export function runPlugin(profile, args) {
    ...
    if (exitCode === 0) {
        ...
        if (args.some(argument => /^git\+|^github:|\.git(?:#|$)/.test(argument))) {
            process.stderr.write(`${NAME}: git-hosted plugins build on install via their prepare script, which pnpm blocks until allowed — `
