...
function resolveBoot(program, profile, options, args) {
    ...
    if (patches.includes(''))
        program.error('error: --patch needs a path');
    if (options.fromDefaultProfile === '')
        program.error('error: --from-default-profile needs a name');
...
export function parseDshArgs(argv, version) {
    ...
    const rejectParentOptions = (command) => {
        ...
        if (parent.profile !== undefined || parent.patch !== undefined
            ...
            program.error(`error: ${command} takes none of parent --profile, --from-default-profile, --patch, --dump-config, or --dump-default-config`);
        }
    };
    ...
    if (resolved === undefined)
        throw new Error('dsh: no invocation resolved');
    return resolved;
}
