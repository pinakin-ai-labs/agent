...
async function runCli() {
	...
	switch (invocation.mode) {
		case "profile": {
			const { runProfile } = await import("./profile-boot-DB8eVKBx.js");
			await runProfile({
		...
		case "plugin": {
			const { runPlugin } = await import("./plugin-Bk_PbPwP.js");
			process.exit(runPlugin(invocation.profile, invocation.args));
			break;
		}
		case "dump-config": {
			const { runDumpConfig } = await import("./dump-config-CpQgpJa2.js");
