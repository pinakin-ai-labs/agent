...
function todoShape(row, panel) {
    const pick = (from, name) => [...from.querySelectorAll('*')].filter(el => hasClass(el, name));
    const first = (from, name) => pick(from, name)[0]?.textContent?.trim() ?? '<absent>';
...
describe('assembled todo surfaces', () => {
    it('renders the parallel plan as a row summary, a separate active count, and the dock plan strip', async () => {
        ...
        if (toggle === null)
            throw new Error('the plan strip must expose its expand toggle');
...
//# sourceMappingURL=todo-row.expected.e2e.js.map