...
it('paints the submission echo on the send keystroke and swaps it for the durable node', async () => {
    ...
    const composer = await waitFor(() => {
        const surface = document.querySelector('[data-composer-input]');
        if (surface === null)
    ...
    const echo = document.querySelector('[data-submission-echo]');
    if (echo === null)
        throw new Error('submission echo missing on the send keystroke');
...
//# sourceMappingURL=submission-echo.e2e.js.map