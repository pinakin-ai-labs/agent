...
import { hasClass, installAssembledBootEnv, mountAssembledApp, REFRESHING_GOLDEN } from "./assembled-boot.js";
...
function cardShape(root) {
    const card = root.querySelector('[data-search]');
    if (card === null)
        return '<no search card>';
    const pick = (from, name) => [...from.querySelectorAll('*')].filter(el => hasClass(el, name));
    const lines = [`kind=${card.getAttribute('data-search')}`];
    const summary = pick(card, 'summary')[0]?.textContent?.trim();
    if (summary !== undefined && summary !== '')
        lines.push(`summary=${summary}`);
    for (const header of pick(card, 'fileHeader'))
        lines.push(`file=${header.textContent?.trim() ?? ''}`);
    for (const row of pick(card, 'line'))
        lines.push(`line=${row.textContent?.trim() ?? ''}`);
    const expand = pick(card, 'expand')[0]?.textContent?.trim();
    if (expand !== undefined && expand !== '')
        lines.push(`expand=${expand}`);
...
//# sourceMappingURL=search-card.expected.e2e.js.map