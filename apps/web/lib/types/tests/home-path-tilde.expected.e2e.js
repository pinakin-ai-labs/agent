...
import { installAssembledBootEnv, mountAssembledApp, REFRESHING_GOLDEN } from "./assembled-boot.js";
...
describe('assembled POSIX home-path display', () => {
    it('shows the home-descendant Workspace path as ~ and copies the full path', async () => {
        ...
        if (group == null)
            throw new Error('home-descendant Workspace group missing');
        fireEvent.pointerEnter(group.parentElement);
        ...
        act(() => { fireEvent.pointerLeave(group.parentElement); });
    });
});
//# sourceMappingURL=home-path-tilde.expected.e2e.js.map