...
async function scrollRowIntoWindow(needle) {
    await waitFor(() => {
    ...
    if (!(pane instanceof HTMLElement))
        throw new Error('trajectory scroll pane missing');
    const findRow = () => [...document.querySelectorAll('tr[data-trajectory-row-key]')]
...
it('renders durable record images in the Trajectory details panel from the shared cache', async () => {
    ...
    if (chatSrc === null || chatSrc === undefined)
        throw new Error('chat gallery image missing');
...
//# sourceMappingURL=trajectory-image-display.expected.e2e.js.map