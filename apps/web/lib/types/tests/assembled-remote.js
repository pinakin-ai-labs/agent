...
const fixtureSource = readFileSync(join(process.cwd(), 'apps/web/tests/fixtures/assembled-remote.fixture.json'), 'utf8');
/** Create one isolated RemoteMock world for an assembled built-client case. */
export function createAssembledRemote(options = {}) {
    ...
    const records = new Map([[
            'fx-alpha',
            structuredClone(fixture.follow.records),
        ]]);
    const nextTurns = new Map([...records].map(([sessionId, sessionRecords]) => {
        let next = 0;
        for (const { event } of sessionRecords) {
            if (event.type !== 'turn/start' || !isRecord(event.data))
                continue;
            const turn = event.data['turn'];
            if (typeof turn === 'number')
                next = Math.max(next, turn + 1);
        }
        return [sessionId, next];
    }));
    const attachments = new Map([['fixture:image', structuredClone(fixture.attachment)]]);
    ...
    mock.stream('$events', (_args, stream) => {
        for (const frame of fixture.remoteEvents)
            stream.push(structuredClone(frame));
    });
    ...
    mock.unary('$events/result', (result) => {
    ...
    mock.unary('session/create', (request) => {
    ...
    mock.unary('session/attachment', (request) => {
        request = recordValue(request, 'request');
    ...
    mock.unary('session/prompt', (request) => {
        request = recordValue(request, 'request');
    ...
    mock.unary('commands/execute', (args) => {
...
function eventOf(seq, type, data, surfaceOp) {
    return {
        type: 'event',
        event: {
            seq,
...
function recordString(value, key) {
    ...
    if (typeof selected !== 'string')
        throw new TypeError(`assembled fixture ${key} must be a string`);
    return selected;
}
...
function recordArray(value, key) {
    ...
    if (!Array.isArray(selected))
        throw new TypeError(`assembled fixture ${key} must be an array`);
    return selected;
}
function recordValue(value, key) {
    if (!isRecord(value) || !(key in value))
        throw new TypeError(`assembled fixture ${key} is missing`);
    return value[key];
}
function isRecord(value) {
    return typeof value === 'object' && value !== null;
}
//# sourceMappingURL=assembled-remote.js.map