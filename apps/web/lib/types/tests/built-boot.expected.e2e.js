...
const clientBuildEnvironment = requireObject(Reflect.get(record, 'environment'), 'client build record environment must be an object');
...
function clientBuildValue(name) {
...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
    ...
    if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
        ...
        if (version === undefined)
            throw new Error('default client build record must carry DSH_CLIENT_VERSION');
...
//# sourceMappingURL=built-boot.expected.e2e.js.map