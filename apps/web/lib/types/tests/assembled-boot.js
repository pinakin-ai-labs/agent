...
const workspacePackageManifests = new Map(globSync('packages/*/*/package.json', { cwd: REPO_ROOT }).map((relative) => {
    ...
    if (pkg.name === undefined)
        throw new Error(`assembled boot: workspace package has no name: ${path}`);
    return [pkg.name, path];
}));
const appBoot = await import(__rewriteRelativeImportExtension(pathToFileURL(webBundleResolver.resolve('@deepseek-ai/dsh-app-boot')).href));
function resolvePackageManifest(specifier) {
    return workspacePackageManifests.get(specifier);
}
function resolveClientExport(packagePath, pkg) {
...
function loadAssembledPlugins() {
    const entries = appBoot.composeEntries(BUNDLE_LAYERS.map(layer => appBoot.loadOverlayPatches('assembled boot', layer.patch)));
    const plugins = new Map();
    for (const entry of entries) {
        if (entry.disabled === true || typeof entry.name !== 'string')
            continue;
...
const BOOTSTRAP_IDS = ['@deepseek-ai/dsh-client-modules'];
/** Build the fixture graph after applying per-scenario package exclusions. */
function bootGraph(plugins) {
    const bootstrapEntries = plugins
        .map(plugin => plugin.id)
        .filter(id => BOOTSTRAP_IDS.includes(id));
...
function bundleTable(graph, plugins) {
    ...
    for (const batch of graph.batches) {
        bundles.set(batch.url, batch.entries.map((id) => {
            ...
            if (plugin === undefined)
                throw new Error(`assembled boot: batch names unknown plugin ${id}`);
            const code = bundles.get(plugin.url);
            if (code === undefined)
                throw new Error(`assembled boot: missing built bundle ${plugin.url}`);
            return code;
        }).join('\n;\n'));
    }
    return bundles;
...
export function installAssembledBootEnv() {
    ...
    afterEach(async () => {
        ...
        if (failures.length > 0)
            throw new AggregateError(failures, 'assembled boot teardown failed');
    });
}
/**
 ...
 */
export function mountAssembledApp(options = {}) {
    ...
    if (bootstrap === undefined)
        throw new Error('missing parser-preloaded fixture batch');
    (0, eval)(bootstrap);
    act(() => {
