...
function noticeShape(row) {
    const first = (name) => [...row.querySelectorAll('*')].filter(el => hasClass(el, name))[0]?.textContent?.trim() ?? '<absent>';
    return [
...
//# sourceMappingURL=max-tokens-notice.expected.e2e.js.map