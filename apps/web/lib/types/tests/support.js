 ...
 */
export async function expandOwningTurnProcess(page, target) {
    const turn = await target.evaluate(element => element.closest('[data-chat-turn]')?.dataset.chatTurn);
    if (turn === undefined || await target.isVisible())
        return;
