...
async function freshComposer() {
    ...
    return await waitFor(() => {
        const surface = document.querySelector('[data-composer-input][data-placeholder="Describe what you want to build, / commands, @ files or sessions"]');
        if (surface === null)
            throw new Error('composer surface missing');
        return surface;
    }, { timeout: 10_000 });
}
...
async function pasteText(surface, text) {
...
async function pasteImage(textarea, name) {
...
it('refuses an image-carrying submit to a non-declaring command and keeps draft and images', async () => {
    ...
    const notice = await waitFor(() => {
        ...
        if (el === undefined)
            throw new Error('composer refusal banner missing');
        return el;
    }, { timeout: 5_000 });
...
//# sourceMappingURL=command-image-envelope.expected.e2e.js.map