...
function messagesResponse(text, complete = true) {
    const events = [
        { type: 'message_start', message: { id: 'web-smoke-response', model: 'mock-model', usage: { input_tokens: 3, output_tokens: 0 } } },
        { type: 'content_block_start', index: 0, content_block: { type: 'text', text } },
    ];
    if (complete)
        events.push({ type: 'content_block_stop', index: 0 }, { type: 'message_delta', delta: { stop_reason: 'end_turn' }, usage: { output_tokens: 1 } }, { type: 'message_stop' });
    return events.map(event => `data: ${JSON.stringify(event)}\n\n`).join('');
}
/** Exchange a printed process token once for Node-side HTTP/WebSocket probes. */
function authenticatedWeb(launchUrl) {
...
const comboMapUrl = (url) => url.replace(/\/client\.js(?=,|&rev=)/g, '/client.js.map');
...
async function remoteRpc(baseUrl, endpoint, args) {
    ...
    if (!response.ok)
        throw new Error(`${endpoint} failed over HTTP ${response.status}: ${await response.text()}`);
...
async function sessionCursor(baseUrl, sessionId) {
    const authenticated = await authenticatedWeb(baseUrl);
        ...
        return await new Promise((resolve, reject) => {
            ...
            const message = (event) => {
                try {
                    ...
                    if (text === undefined)
                        throw new Error('session/follow published a non-text frame');
                    const frame = JSON.parse(text);
                    if (!isRecord(frame) || frame.streamId !== streamId)
                        return;
                    if (frame.type === 'error') {
            ...
            const failed = () => { finish(new Error('session/follow carrier failed before its opening cursor')); };
            const closed = () => { finish(new Error('session/follow carrier closed before its opening cursor')); };
...
async function history(baseUrl, sessionId) {
    ...
    return remoteRpc(baseUrl, 'session/page', {
...
function firstMappedSource(script, payload) {
    ...
    for (let line = 0; line < lines.length; line++) {
        ...
        for (let column = 0; column <= lastColumn; column++) {
            ...
            if (!('originalSource' in entry) || typeof entry.originalSource !== 'string')
                continue;
...
describe('dsh web keyless CLI smoke', () => {
    it('serves a usable app from two immutable plugin batches', async () => {
        ...
        try {
            ...
            expect(batchPaths).toContainEqual(expect.stringMatching(/^\/plugins\/\?\?.+\/client\.js,.+\/client\.js&rev=[a-f\d]{12}$/));
            expect(batchPaths).toContainEqual(expect.stringMatching(/^\/plugins\/\?\?@deepseek-ai\/dsh-client-modules\/client\.js&rev=[a-f\d]{12}$/));
            ...
            for (const path of batchPaths) {
                ...
                expect(sections.every(section => (Array.isArray(section.map.sources)
                    && Array.isArray(section.map.sourcesContent)
                    && section.map.sourcesContent.length === section.map.sources.length
                    && section.map.sourcesContent.every(source => typeof source === 'string')))).toBe(true);
    ...
    it('routes web runtime context and workspace instructions through the real CLI request', async () => {
        ...
        try {
            ...
            const created = await remoteRpc(baseUrl, 'session/create', { request: {} });
            await remoteRpc(baseUrl, 'session/prompt', { request: {
    ...
    it('retries a partial transport failure through the shipped Web composition', async () => {
        ...
        try {
            ...
            const retry = page.records.find(record => (record.type === 'event' && record.event.type === 'llm/retry'));
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
    ...
    it('empty-state first send completes a real model round', async () => {
        ...
        await expect.poll(async () => (await remoteRpc(baseUrl, 'session/list', { _request: {} })).items.length, {
            timeout: 15_000,
        }).toBe(1);
        const sessions = await remoteRpc(baseUrl, 'session/list', { _request: {} });
