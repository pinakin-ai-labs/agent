...
import { existsSync, fsyncSync, ftruncateSync, lstatSync, mkdirSync, openSync, closeSync, readFileSync, readdirSync, realpathSync, unlinkSync, writeFileSync, writeSync, } from 'node:fs';
import { delimiter, dirname, join, resolve, sep } from 'node:path';
import { DESKTOP_HOST_PACKAGE, desktopCorePackageOverrides, verifyDesktopCorePackageSet, } from "./core-package-set.js";
import { removeOwnedDirectory } from "./owned-directory.js";
import { desktopRuntimeId, readDesktopRuntime } from "./runtime-tree.js";
import { desktopPluginLockHash, linkDesktopHostPackages, readDesktopProfileState, recordDesktopRuntimeProfile, unlinkDesktopHostPackages, validateDesktopPluginGraph, } from "./profile-packages.js";
...
function assertPackageName(name) {
    if (!PACKAGE_NAME_PATTERN.test(name))
        throw new Error(`desktop project: invalid npm package name ${JSON.stringify(name)}`);
}
function assertVersion(version) {
    if (!VERSION_PATTERN.test(version))
        throw new Error(`desktop project: invalid exact version ${JSON.stringify(version)}`);
}
/**
...
export function packageNameFromSpec(spec) {
    ...
    if (spec.startsWith('@')) {
        ...
        if (slash === -1)
            throw new Error(`desktop project: invalid scoped package spec ${JSON.stringify(spec)}`);
        const versionAt = spec.indexOf('@', slash);
        const name = versionAt === -1 ? spec : spec.slice(0, versionAt);
        assertPackageName(name);
        if (versionAt !== -1)
            assertVersion(spec.slice(versionAt + 1));
        return name;
    }
...
function projectManifest(projectDir) {
    ...
    const manifest = { ...value, dependencies: value.dependencies ?? {} };
...
export class DesktopProjectManager {
    ...
    releaseVersion() {
        ...
        if (state === undefined)
            throw new Error('desktop project: active profile has no runtime state');
        return state.version;
    }
    /** Reject a profile whose dependency links were prepared for another runtime. */
    assertProfileRuntime(projectDir) {
        if (existsSync(this.pendingPackages))
            throw new Error('desktop project: package preparation is incomplete; retry startup');
    ...
    get pendingPackages() { return join(this.paths.profile, 'desktop-packages-pending'); }
    currentRuntime() {
        if (this.descriptor === undefined)
            throw new Error('desktop project: runtime metadata has not been loaded');
    ...
    prepareProfile(projectDir) {
        ...
        if (resolutionMode === 'runtime')
            recordDesktopRuntimeProfile(projectDir, runtime);
        else
    ...
    async mutate(mutation, hooks) {
        await this.withLock(async () => {
            this.currentRuntime();
            if (!existsSync(this.paths.profile))
                throw new Error('desktop project: active profile is not installed');
            await hooks.beforeChange();
            ...
            if (packagesChanged && this.runtime.profileResolution !== 'runtime')
                unlinkDesktopHostPackages(this.paths.profile);
            try {
                ...
                if (packagesChanged) {
                    const runtime = this.currentRuntime();
                    if (this.runtime.profileResolution === 'runtime')
                        recordDesktopRuntimeProfile(this.paths.profile, runtime);
                    else
                        linkDesktopHostPackages(this.paths.profile, this.runtime.dsh, runtime);
                }
            }
    ...
    async applyMutation(projectDir, mutation) {
        switch (mutation.type) {
            case 'plugin-add': {
                ...
                writeProfilePlugins(projectDir, [...current, installed].sort((left, right) => left.name.localeCompare(right.name)));
                return;
            }
            case 'plugin-remove': {
            ...
            case 'plugin-toggle': {
                ...
                if (!plugins.some(plugin => plugin.name === mutation.name))
                    throw new Error(`desktop project: plugin ${mutation.name} is not installed`);
                writeProfilePlugins(projectDir, plugins.map(plugin => (plugin.name === mutation.name ? { ...plugin, enabled: mutation.enabled } : plugin)));
                return;
            }
            default:
                mutation;
    ...
    async runPnpm(projectDir, args) {
        ...
        if (command === undefined)
            throw new Error('desktop project: pnpm command is required');
        ...
        await new Promise((settle, reject) => {
            ...
            child.once('close', (code, signal) => {
                complete(() => {
                    ...
                    reject(new Error(`desktop project: pnpm exited with ${String(code ?? signal)}${diagnostics.trim() === '' ? '' : `: ${diagnostics.trim()}`}`));
                });
            });
            try {
                if (child.pid === undefined)
                    throw new Error('desktop project: pnpm did not report a process id');
    ...
    writeLockOwner(pid) {
        ...
        if (descriptor === undefined)
            throw new Error('desktop project: package transaction lost its lock');
    ...
    async withLock(operation) {
        ...
        if (lstatSync(this.paths.profile).isSymbolicLink())
            throw new Error('desktop project: profile directory must not be a link');
        let descriptor;
        try {
            ...
            if (error.code === 'EEXIST') {
                ...
                if (active)
                    throw new Error('desktop project: another package transaction is active');
...
export function createRuntimeProjectMetadata(projectDir, release) {
    ...
    writeFileSync(join(projectDir, 'pnpm-workspace.yaml'), workspaceFile(desktopCorePackageOverrides(packageSet)), { mode: 0o600 });
}
/**
 ...
 */
export function createDevelopmentProjectMetadata(projectDir, release) {
    mkdirSync(projectDir, { recursive: true, mode: 0o700 });
    const manifest = {
        name: PROJECT_NAME,
        private: true,
...
export function createPluginProfile(projectDir) {
    mkdirSync(projectDir, { recursive: true, mode: 0o700 });
    writeJson(join(projectDir, 'package.json'), {
