...
import { DESKTOP_HOST_PACKAGE } from "./core-package-set.js";
import { parseDesktopRelease } from "./release.js";
 ...
 */
export function runtimePath(root, path) {
    ...
    const visit = (directory) => {
        for (const entry of readdirSync(directory, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                ...
                throw new Error(`desktop runtime: unsupported filesystem entry ${name}`);
        }
    };
    visit(root);
    return files;
}
function runtimeFile(path, name, body) {
 ...
 */
export function inventoryDesktopRuntime(root) {
    return runtimeFiles(root).map(({ path, name }) => runtimeFile(path, name, readFileSync(path)))
        .sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0);
}
async function inventoryRuntimeForVerification(root) {
 ...
 */
export function writeDesktopRuntime(root, release, sharedNames, target = process) {
    const sharedPackages = [...new Set(sharedNames)].sort().map((name) => {
        if (!PACKAGE_NAME.test(name))
            throw new Error(`desktop runtime: invalid shared package ${name}`);
        const path = `node_modules/${name}`;
        const manifest = JSON.parse(readFileSync(join(runtimePath(root, path), 'package.json'), 'utf8'));
 ...
 */
export function readDesktopRuntime(root) {
    ...
    return { schemaVersion: value.schemaVersion, release, platform: value.platform,
 ...
 */
export async function verifyDesktopRuntime(root, electronVersion, target = process) {
    ...
    if (release.version !== electronVersion)
    ...
    for (const entry of descriptor.sharedPackages) {
        const manifest = JSON.parse(readFileSync(join(runtimePath(root, entry.path), 'package.json'), 'utf8'));
    ...
    // Windows has no portable Unix executable permission bits.
    const comparable = (items) => process.platform === 'win32'
 ...
 */
export function desktopRuntimeId(descriptor) {
    return createHash('sha256').update(JSON.stringify(descriptor)).digest('hex');
}
//# sourceMappingURL=runtime-tree.js.map