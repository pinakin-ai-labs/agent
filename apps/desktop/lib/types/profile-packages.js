...
import { desktopRuntimeId, runtimePath } from "./runtime-tree.js";
 ...
 */
export function readDesktopProfileState(profile) {
    ...
    if (new Set(links.map(link => link.name)).size !== links.length)
        throw new Error('desktop profile: duplicate managed link');
 ...
 */
export function desktopPluginLockHash(profile) {
 ...
 */
export function unlinkDesktopHostPackages(profile) {
 ...
 */
export function linkDesktopHostPackages(profile, root, runtime) {
    ...
    for (const link of links) {
        const path = join(profile, 'node_modules', link.name);
        if (stat(path) !== undefined)
            throw new Error(`desktop profile: plugin installed reserved host package ${link.name}`);
        mkdirSync(dirname(path), { recursive: true });
        symlinkSync(link.target, path, process.platform === 'win32' ? 'junction' : 'dir');
    }
    const state = { schemaVersion: 1, runtimeId: desktopRuntimeId(runtime), version: runtime.release.version,
 ...
 */
export function recordDesktopRuntimeProfile(profile, runtime) {
...
function manifest(path) {
    ...
    if (record(value.peerDependenciesMeta)) {
        for (const [name, meta] of Object.entries(value.peerDependenciesMeta)) {
            if (record(meta) && meta.optional === true)
...
function packageFrom(anchor, name) {
    if (!PACKAGE_NAME.test(name))
        throw new Error(`desktop profile: invalid package name ${name}`);
 ...
 */
export function validateDesktopPluginGraph(profile, root, runtime, activePlugins, resolutionMode = 'link') {
    ...
    const shared = new Map(runtime.sharedPackages.map((entry) => {
        ...
        return [entry.name, { path: canonical, version: entry.version }];
    }));
    if (resolutionMode === 'link') {
        for (const [name, entry] of shared) {
            if (packageFrom(profile, name) !== entry.path)
                throw new Error(`desktop profile: missing or incorrect host link ${name}`);
        }
    }
    ...
    const scan = (modules) => {
        ...
        if (lstatSync(modules).isSymbolicLink())
            throw new Error(`desktop profile: linked package container ${modules}`);
        ...
        for (const entry of readdirSync(modules, { withFileTypes: true })) {
            ...
            if (!inside(profileRoot, canonical))
                throw new Error(`desktop profile: package resolves outside profile: ${path}`);
    ...
    const visit = (path, chain) => {
        ...
        for (const [name, range] of Object.entries({ ...deps, ...info.peerDependencies })) {
            ...
            if (host !== undefined && name in deps)
                throw new Error(`desktop profile: ${chain} must declare ${name} as a peer dependency`);
            if (host !== undefined) {
    ...
    for (const name of activePlugins) {
        ...
        if (path === undefined || !inside(profileRoot, path))
            throw new Error(`desktop profile: missing local plugin ${name}`);
        visit(path, name);
    }
}
//# sourceMappingURL=profile-packages.js.map