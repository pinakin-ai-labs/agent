 ...
 */
export function removeOwnedDirectory(path) {
    ...
    if (!stat.isDirectory())
        throw new Error(`desktop project: owned directory path is not a directory: ${path}`);
    // Electron's recursive rm follows nested Windows junctions into installed resources.
    for (const entry of readdirSync(path, { withFileTypes: true })) {
        const child = join(path, entry.name);
        if (entry.isDirectory())
            removeOwnedDirectory(child);
        else
            unlinkSync(child);
    }
    rmdirSync(path);
}
//# sourceMappingURL=owned-directory.js.map